...
func (s *Service) RegisterTools() error {
	return s.tools.Register(
		...
		tool.Definition{Name: "content.list_categories", Description: "List blog categories and aggregate post counts.", Parameters: schema(`{}`), Surfaces: []string{"agent", "external"}, Risk: tooldomain.ToolRiskRead, Scope: &tool.ScopeRule{Discovery: true, OutputResourceType: "category", OutputKeys: []string{"id"}}, Execute: s.listCategories},
	)
}
