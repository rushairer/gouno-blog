...
func providerStarterWorkflows() []providerStarterWorkflow {
	...
	imageBrief := json.RawMessage(`{"type":"object","additionalProperties":false,"required":["post_ids","format"],"properties":{"post_ids":{"title":"文章","type":"array","items":{"type":"integer"},"minItems":1,"maxItems":20,"x-gouno-resource":"post","x-gouno-widget":"entity-multi-select"},"format":{"title":"图片任务","type":"string","enum":["image_brief"],"default":"image_brief"}}}`)
	...
	return []providerStarterWorkflow{
		...
		{key: "selected_article_image_generation", name: "生成封面/文配图（手选）", description: "为手选文章生成图片 Brief；审批后调用默认图片 Provider，随后由你选择并应用到封面或正文。", agentKey: "content_distribution", inputSchema: imageBrief, scopePolicy: strict, steps: func(id int64) []domain.WorkflowStep { return batchSteps("/input/post_ids", 20, id, true) }},
