...
func TestScheduledResourceQueryRetryKeepsSnapshotAndScope(t *testing.T) {
	...
	service := &Service{db: db, admission: newTestRunAdmissionCoordinator(db), mediaRuns: newTestMediaRunCoordinator(db), definitions: workflowrepository.NewDefinitionRepository(db), catalog: NewResourceCatalog(db)}
...
func TestResourceQueryEmptyPolicyCanFailWithoutAgentRun(t *testing.T) {
	...
	service := &Service{db: db, admission: newTestRunAdmissionCoordinator(db), mediaRuns: newTestMediaRunCoordinator(db), definitions: workflowrepository.NewDefinitionRepository(db), catalog: NewResourceCatalog(db), agents: newResourceQueryManagement(db)}
...
func TestForEachCanAggregatePartialFailures(t *testing.T) {
	...
	t.Cleanup(func() { cleanupResourceQueryFixture(t, ctx, db, workflowID, versionID, 0) })
	service := &Service{db: db, admission: newTestRunAdmissionCoordinator(db), mediaRuns: newTestMediaRunCoordinator(db), definitions: workflowrepository.NewDefinitionRepository(db), agents: newResourceQueryManagement(db)}
	var runID int64
...
func TestRetryFailedForEachIterationUsesOriginalInput(t *testing.T) {
	...
	t.Cleanup(func() { cleanupResourceQueryFixture(t, ctx, db, workflowID, versionID, 0) })
	service := &Service{db: db, admission: newTestRunAdmissionCoordinator(db), mediaRuns: newTestMediaRunCoordinator(db), definitions: workflowrepository.NewDefinitionRepository(db)}
	var runID int64
