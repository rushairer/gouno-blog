-- Migration 046 may have run before a Provider profile was configured. In that
-- case its optional resource-oriented Agents could not be created, leaving two
-- starter Workflows without a fixed Agent and two dependent templates absent.
-- Backfill only missing platform-owned records; existing Agent and Workflow
...
WITH templates(template_key,agent_name,input_schema,steps) AS (
    VALUES
