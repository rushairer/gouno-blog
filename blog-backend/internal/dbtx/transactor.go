...
// Transactor coordinates database transactions shared across capability
// boundaries. It belongs to infrastructure rather than any business repository.
...
func NewTransactor(db *sql.DB, logger *zap.Logger) *Transactor {
	if db == nil {
		panic("dbtx.NewTransactor: db is required")
	}
	if logger == nil {
		logger = zap.NewNop()
	}
