...
type CommunityController struct {
	svc             *communityservice.CommunityService
	limiter         ratelimit.Limiter
	fallbackLimiter ratelimit.Limiter
	visitorTokens   *communityservice.VisitorTokenManager
	logger          *zap.Logger
}

func NewCommunityController(svc *communityservice.CommunityService, limiter ratelimit.Limiter, visitorSecret string, logger *zap.Logger) *CommunityController {
	if logger == nil {
		logger = zap.L()
	}
	return &CommunityController{
		svc:             svc,
		limiter:         limiter,
		fallbackLimiter: ratelimit.NewMemoryLimiter(),
		visitorTokens:   communityservice.NewVisitorTokenManager(visitorSecret),
		logger:          logger,
	}
}

func (ctrl *CommunityController) actor(c *gin.Context) communityservice.Actor {
	if principal, ok := c.Get("blog_principal_id"); ok {
		if principalID, principalOK := principal.(int64); principalOK && principalID > 0 {
			...
			return communityservice.Actor{Key: "principal:" + strconv.FormatInt(principalID, 10), PrincipalID: principalID, DisplayName: name, Authenticated: true}
		}
	}
	visitorID := ctrl.visitorID(c)
	return communityservice.Actor{Key: "anon:" + visitorID, DisplayName: "Anonymous"}
}
...
func (ctrl *CommunityController) allow(c *gin.Context, operation string, actor communityservice.Actor, limit int, window time.Duration) bool {
...
// GetAllComments serves the moderation-only route and keeps Community as the
// canonical owner of comment reads, including hidden/pending entries.
func (ctrl *CommunityController) GetAllComments(c *gin.Context) {
	post, err := ctrl.svc.ResolvePublishedPost(c.Request.Context(), c.Param("slugOrID"))
	if err != nil {
		controllerutil.WriteDomainError(c, err)
		return
	}
	comments, err := ctrl.svc.GetAllComments(c.Request.Context(), post.ID)
	if err != nil {
		controllerutil.WriteDomainError(c, err)
		return
	}
...
func principalID(c *gin.Context) int64 {
	value, _ := c.Get("blog_principal_id")
	id, _ := value.(int64)
	return id
}
...
func (ctrl *CommunityController) ListNotifications(c *gin.Context) {
	...
	page, pageSize = controllerutil.NormalizePagination(page, pageSize, 30)
