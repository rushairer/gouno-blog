...
func (r *CommunityRepository) SetLike(ctx context.Context, postID int64, actorKey string, liked bool) (*domain.State, error) {
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
		return nil, err
	}
	...
	state := &domain.State{Liked: liked}
...
func (r *CommunityRepository) CommunityState(ctx context.Context, postID int64, actorKey, _ string) (*domain.State, error) {
	state := &domain.State{}
