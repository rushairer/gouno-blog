			...
			for _, spec := range gen.Specs {
				...
				case *ast.TypeSpec:
					if _, retired := retiredSymbols[typed.Name.Name]; retired {
						t.Errorf("%s redeclares Taxonomy-owned symbol %s in root internal/domain", path, typed.Name.Name)
					}
				case *ast.ValueSpec:
					for _, name := range typed.Names {
						if _, retired := retiredSymbols[name.Name]; retired {
							t.Errorf("%s redeclares Taxonomy-owned symbol %s in root internal/domain", path, name.Name)
						}
					}
				}
			}
	...
	err = filepath.WalkDir(backendRoot, func(path string, entry fs.DirEntry, walkErr error) error {
		...
		ast.Inspect(parsed, func(node ast.Node) bool {
			...
			if _, rootAlias := rootAliases[ident.Name]; rootAlias {
				t.Errorf("%s consumes Taxonomy-owned symbol %s through root internal/domain; import internal/taxonomy/domain instead", path, selector.Sel.Name)
			}
			return true
		})
		return nil
