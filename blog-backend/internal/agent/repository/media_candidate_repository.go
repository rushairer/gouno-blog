...
func NewMediaCandidateRepository(db *sql.DB) *MediaCandidateRepository {
...
func (r *MediaCandidateRepository) ListMediaCandidates(ctx context.Context) ([]*domain.MediaCandidate, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.PostID, &item.SourceRunID, &item.SourceApprovalID, &item.Headline,
			...
			&item.SafetyStatus, &item.CopyrightStatus, &item.AltText, &item.ReviewedByPrincipalID, &item.ReviewNote,
...
func (r *MediaCandidateRepository) SetMediaGenerationInstruction(ctx context.Context, id int64, instruction string) error {
...
func (r *MediaCandidateRepository) GetMediaCandidate(ctx context.Context, id int64) (*domain.MediaCandidate, error) {
...
func (r *MediaCandidateRepository) ListMediaCandidatesByWorkflowRun(ctx context.Context, runID int64) ([]*domain.MediaCandidate, error) {
...
func (r *MediaCandidateRepository) SelectMediaCandidate(ctx context.Context, id int64, placement, anchor string) error {
	if placement == "" {
		placement = "cover"
	}
...
func (r *MediaCandidateRepository) SelectMediaCandidates(ctx context.Context, selections []domain.MediaCandidateSelection) error {
...
func (r *MediaCandidateRepository) MarkMediaCandidateApplied(ctx context.Context, id, versionID int64) error {
...
func (r *MediaCandidateRepository) SyncPostVersionToken(ctx context.Context, postID int64, token string) error {
...
func (r *MediaCandidateRepository) AttachMediaAsset(ctx context.Context, candidateID, mediaAssetID int64) error {
...
func (r *MediaCandidateRepository) ClaimMediaGeneration(ctx context.Context, id int64) (*domain.MediaCandidate, error) {
	var item domain.MediaCandidate
	err := r.db.QueryRowContext(ctx, `UPDATE ai_media_candidates SET generation_status='generating',generation_attempt=generation_attempt+1,error_code=NULL,error_message=NULL,generation_started_at=NOW(),generation_deadline_at=NOW()+INTERVAL '15 minutes',cancelled_at=NULL
		...
		generation_status,safety_status,copyright_status,alt_text,reviewed_by_principal_id,review_note,reviewed_at,created_at,generation_attempt,regeneration_instruction`, id).Scan(
		...
		&item.SafetyStatus, &item.CopyrightStatus, &item.AltText, &item.ReviewedByPrincipalID, &item.ReviewNote, &item.ReviewedAt, &item.CreatedAt, &item.GenerationAttempt, &item.RegenerationInstruction)
	return &item, err
}
...
func (r *MediaCandidateRepository) CancelMediaGeneration(ctx context.Context, candidateID int64) error {
...
func (r *MediaCandidateRepository) ReviewMediaCandidate(ctx context.Context, id int64, action string, reviewerPrincipalID int64, note string) error {
	...
	result, err := r.db.ExecContext(ctx, `UPDATE ai_media_candidates SET generation_status=$2,safety_status=$3,
		copyright_status=$4,reviewed_by_principal_id=$5,review_note=$6,reviewed_at=NOW()
		WHERE id=$1 AND generation_status='brief_ready'`, id, status, safety, copyright, reviewerPrincipalID, note)
	if err != nil {
		return err
	}
	if affected, _ := result.RowsAffected(); affected == 0 {
...
func (r *MediaCandidateRepository) RejectMediaCandidate(ctx context.Context, id int64, reason string) error {
	if reason == "" {
...
func (r *MediaCandidateRepository) RejectMediaCandidates(ctx context.Context, ids []int64) error {
	if len(ids) == 0 {
		return nil
	}
	tx, err := r.db.BeginTx(ctx, nil)
