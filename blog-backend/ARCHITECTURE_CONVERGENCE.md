...
| `internal/controller` Agent/Operations/Knowledge controllers | Transitional HTTP ownership | Active router/controller composition; Workflow transport is capability-local | Move the remaining capability HTTP owners only after their service boundaries are stable; preserve routes, middleware, BFF and authorization behavior exactly. |
...
- **A — migrate to capability / retire facade:** Agent/Operations/Knowledge controllers after their service contracts stabilize. Workflow HTTP ownership is canonical under `internal/workflow/controller`. Flat Agent repository delegates are retired.
...
1. **Controller / composition-root convergence:** Workflow HTTP ownership is converged; move Agent/Operations/Knowledge HTTP ownership in coherent capability slices while preserving routes, middleware, BFF and authorization semantics exactly.
...
Completed slices: **Agent Approval / Media Candidate / Workflow Event orchestration**, **Agent ManagementService dependency cutover**, **Starter Pack application coordination**, **Runner dependency/lifecycle convergence**, **Workflow Definition/Version persistence extraction**, **Workflow Run lifecycle coordination**, **Agent MediaCandidate / Workflow Run boundary convergence**, and **Workflow Run admission/retry/recovery convergence**, **Workflow dispatch/execution persistence convergence**, and **Workflow read-model classification**, and **Workflow HTTP ownership convergence** now use canonical ownership boundaries and explicit composition-root wiring. The flat `repository.AgentRepository` aggregate and its Agent/Workflow/Notification delegates are retired; `internal/repository` remains only for the Connector-held Transactor alias until that Hold is explicitly lifted.
