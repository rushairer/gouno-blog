...
| `internal/controller` Agent/Knowledge controllers | Transitional HTTP ownership | Active router/controller composition; Workflow and Operations transport are capability-local | Move the remaining capability HTTP owners only after their service boundaries are stable; preserve routes, middleware, BFF and authorization behavior exactly. |
...
- **A — migrate to capability / retire facade:** Agent/Knowledge controllers after their service contracts stabilize. Workflow and Operations HTTP ownership are canonical under their capability controllers. Flat Agent repository delegates are retired.
...
1. **Controller / composition-root convergence:** Workflow and Operations HTTP ownership are converged; move Agent then Knowledge HTTP ownership in coherent capability slices while preserving routes, middleware, BFF and authorization semantics exactly.
2. **Transitional layer retirement:** delete remaining flat controller facades and stale adapters only after consumer proof and full gates.
3. **Shared-model classification:** classify remaining root `internal/domain` models only where a real capability/shared-kernel decision is needed; never move them for directory symmetry.

Completed slices: **Agent Approval / Media Candidate / Workflow Event orchestration**, **Agent ManagementService dependency cutover**, **Starter Pack application coordination**, **Runner dependency/lifecycle convergence**, **Workflow Definition/Version persistence extraction**, **Workflow Run lifecycle coordination**, **Agent MediaCandidate / Workflow Run boundary convergence**, and **Workflow Run admission/retry/recovery convergence**, **Workflow dispatch/execution persistence convergence**, and **Workflow read-model classification**, and **Workflow HTTP ownership convergence**, and **Operations HTTP ownership convergence** now use canonical ownership boundaries and explicit composition-root wiring. The flat `repository.AgentRepository` aggregate and its Agent/Workflow/Notification delegates are retired; `internal/repository` remains only for the Connector-held Transactor alias until that Hold is explicitly lifted.
...
- The transitional flat Agent controller now consumes only a narrow Workflow lifecycle/reconciliation port for approval/media cross-capability coordination; it is not the Workflow HTTP aggregate.
