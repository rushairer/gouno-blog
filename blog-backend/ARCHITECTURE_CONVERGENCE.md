...
As of 2026-09-14, all **non-held** Capability Module migration work is complete. There is no active non-held flat-layer, shared-model, controller-ownership, repository-ownership, transaction-ownership, or composition-root migration debt.

The only retained compatibility boundaries are the stable Access security boundary and the explicitly deferred Connector Module Hold. They are classified held boundaries with explicit consumers, guards, and removal conditions; they are **not** authorization to continue opportunistic architecture migration. Connector work still requires an explicit user instruction naming Connector work, and Access/Auth BFF security behavior remains frozen unless explicitly requested.

The dated convergence sections later in this document are chronological migration records. Statements such as "remaining work" inside those historical checkpoints describe the repository state at that date/slice; the table and terminal status in this section are authoritative for the current backlog.
...
| `internal/controller` minimal response compatibility helper | Held-boundary compatibility facade | Access uses `WriteDomainError`; Connector-held transport uses `WriteDomainError` and `ParamPositiveID` | Delete when the stable Access boundary and Connector-held transport leave the flat controller package; no new consumers are allowed. |
...
## Remaining held-boundary conditions

There is no active **non-held** Capability Module migration debt.

1. **Connector Module Hold:** the Connector-held controller shell/transport and flat Transactor alias remain intentionally frozen. Retirement requires an explicit Connector instruction, direct `dbtx` cutover, dedicated security review, unchanged externally observable behavior, and green Connector/BFF/repository gates.
2. **Access stable security boundary:** Access remains intentionally stable rather than migration debt. Do not move or reshape its flat transport merely to eliminate a directory; any future boundary change requires an explicit security-focused slice with authorization and behavior proof.
3. **Architecture guard maintenance:** root `internal/domain` and `internal/service` remain retired; flat repository/controller allowlists must not grow. A newly required compatibility facade must be explicitly classified, narrowly consumed, guarded, and documented with a concrete removal condition in the same change.

Completed slices: **Agent Approval / Media Candidate / Workflow Event orchestration**, **Agent ManagementService dependency cutover**, **Starter Pack application coordination**, **Runner dependency/lifecycle convergence**, **Workflow Definition/Version persistence extraction**, **Workflow Run lifecycle coordination**, **Agent MediaCandidate / Workflow Run boundary convergence**, and **Workflow Run admission/retry/recovery convergence**, **Workflow dispatch/execution persistence convergence**, and **Workflow read-model classification**, and **Workflow HTTP ownership convergence**, and **Operations HTTP ownership convergence**, and **Agent HTTP ownership convergence**, and **Knowledge HTTP ownership convergence**, and **PostVersion restore ownership convergence**, and **Feed HTTP ownership convergence**, and **flat-controller boundary freeze**, and **Page domain alias retirement**, and **Operations domain model ownership convergence**, and **Taxonomy domain model ownership**, and **Analytics read-model ownership**, and **Media domain model ownership**, and **PostVersion domain model ownership**, and **Post domain model ownership**, and **Workflow domain model ownership**, and **Provider domain model ownership**, and **Knowledge EmbeddingProfile domain ownership**, and **ToolRisk domain ownership**, and **Agent domain ownership / root-domain retirement**, and **flat-controller compatibility surface shrink** now use canonical ownership boundaries and explicit composition-root wiring. The flat `repository.AgentRepository` aggregate and its Agent/Workflow/Notification delegates are retired; `internal/repository` remains only for the Connector-held Transactor alias until that Hold is explicitly lifted.
