...
As of 2026-09-14, Capability Module structural convergence is complete. The formerly generic root `internal/domain`, `internal/service`, `internal/repository`, and `internal/controller` buckets are all retired; there is no remaining flat-layer compatibility facade or held structural migration debt.

Access HTTP transport is canonical under `internal/access/controller`. Connector HTTP transport is canonical under `internal/connector/controller`, and Connector consumes `internal/dbtx.Transactor` directly. This structural convergence does not change Access/Auth BFF security semantics or Connector OAuth/callback/credential/outbox/Sandbox behavior. The Connector product-behavior Hold remains active for feature or behavior changes.

The dated convergence sections later in this document are chronological migration records. Historical references to a flat controller/repository boundary or Connector structural Hold describe earlier repository states; this terminal section and the current table are authoritative.
...
| **Workflow** | Workflow definitions/versions/runs/steps/resources/events/interactions; `ai_workflows`, `ai_workflow_versions`, `ai_workflow_runs`, `ai_workflow_step_runs`, `ai_workflow_run_resources`, `ai_workflow_events`, `workflow_interaction_tasks`, `workflow_run_events` | `internal/workflow` plus `internal/workflow/{domain,repository,controller}` | Uses Agent Runner/Management and Tool registry. Agent consumes canonical Workflow interaction/event/scope contracts; Workflow consumes Agent-owned Media Candidate state only through the consumer-sized `MediaCandidateRunStore`. | `RunLifecycle` owns cancellation/deletion; `MediaRunCoordinator` owns MediaCandidate-to-Workflow decisions at Serializable isolation; `RunAdmissionCoordinator` owns queued-run admission/retry snapshots; `DispatchCoordinator` owns due-schedule claims; `ExecutionCoordinator` owns interaction/event atomicity and execution-state persistence through `dbtx.Transactor` where multi-write consistency is required. Repositories receive coordinator transactions. | **Definition/version, lifecycle, MediaCandidate boundary, Run admission/retry/recovery, dispatch claiming, execution checkpoint persistence, read-model classification, HTTP ownership, and domain model ownership are converged.** |
| **Operations** | Operational suggestions, health evidence, candidate sets, feedback, editorial/reply/news work; `ai_operational_suggestions`, `ai_link_health_jobs`, `ai_link_health_snapshots`, `ai_content_candidate_sets`, `ai_content_candidates`, `ai_feedback`, `ai_editorial_tasks`, `ai_comment_reply_drafts`, `ai_daily_news_jobs`, `ai_daily_news_runs`, `ai_daily_news_sources` | `internal/operations` service/controller plus `internal/operations/domain`; no artificial repository layer | Narrow `GovernanceToolCallWriter` -> Agent Run persistence and `GovernanceApprovalWriter` -> Agent Approval persistence; canonical Post service. | Operations service owns governance transactions through `dbtx.Transactor`. | **Service dependency, approval-effect ownership, HTTP ownership, and domain model ownership are converged.** Governance uses narrow Agent writers, approved candidate/task/reply/suggestion effects are persisted by Operations rather than Agent ApprovalRepository, and Operations transport is capability-owned under `internal/operations/controller`. |
| **Knowledge** | Embedding profiles, index jobs/chunks, retrieval metrics/evaluation; `ai_embedding_profiles`, `ai_content_index_jobs`, `ai_content_chunks`, `ai_retrieval_metrics`, `ai_retrieval_eval_cases` | `internal/knowledge` service/controller plus `internal/knowledge/domain`; cohesive service-local persistence is intentional today | Provider HTTP/runtime boundary; Tool consumers through Knowledge service API. | Knowledge service owns multi-write index transactions through `dbtx.Transactor`. | **Service, HTTP ownership, and EmbeddingProfile domain ownership are converged.** Embedding/index transport is capability-owned; persistence/transaction behavior is unchanged. |
| **Media** | Media assets; `media_assets` | `internal/media/{domain,repository,service,controller}` | Access policy and Agent/Operations consumers use Media-owned value contracts; Agent generation/approval uses narrow Media behavior contracts. | Media repository/service. | **Canonical implementation and domain model ownership converged.** |
| **Post** | Post lifecycle and post-owned relationships; principally `posts` plus owned relationship writes | `internal/post/{domain,repository,service,controller}` | Community, Recommendation, Analytics, Tool and Agent use canonical service or narrow readers. | Post service/repository. | **Canonical implementation and domain model ownership converged.** Flat Post facades and the old flat service bucket are retired. |
| **Page** | Pages; `pages` | `internal/page/{domain,repository,service,controller}` | Feed, Agent and Tool use canonical service; Access/Agent/Tool consumers that need Page values import `internal/page/domain` directly. | Page service/repository. | **Canonical implementation; root Page compatibility aliases retired.** |
| **Community** | Comments, reactions, reports and community notification behavior; `comments`, `post_reactions`, `comment_reports`, community-origin rows in `notifications` | `internal/community/{domain,repository,service,controller}` | Narrow published-Post resolution; Tool moderation reads. | Community service/repository. | **Canonical implementation and dependency contracts converged.** The physical `notifications` table is shared with the system-notification persistence boundary; shared storage does not imply duplicate business ownership. |
| **Taxonomy** | Categories, Tags and taxonomy relationships; `categories`, `tags`, association writes | `internal/taxonomy/{domain,repository,service,controller}` | Post/feed consumers use canonical taxonomy API. `Category` and `TagSummary` values are capability-owned. | Taxonomy service/repository. | **Canonical implementation and domain model ownership converged.** |
| **Site** | Site settings; `site_settings` | `internal/site/{repository,service,controller}` | Feed uses a narrow settings-read contract. | Site service/repository. | **Canonical implementation and Feed read contract converged.** |
| **Feed** | RSS and sitemap public read models; no owned persistence table | `internal/feed/controller` | Narrow Post listing, published Page listing and Site Settings reads. | Read-only HTTP composition; transaction ownership remains with called capabilities. | **Canonical HTTP ownership; no flat Feed controller remains.** |
| **Analytics** | Analytics events and summary read model; `analytics_events` | `internal/analytics/{domain,repository,service,controller}` | Cross-capability read model; Tool binds directly to canonical Analytics service and imports the Analytics-owned summary contract. | Analytics service/repository. | **Canonical implementation and read-model ownership converged.** |
| **Recommendation** | Related-post read model; no owned write table | `internal/recommendation/{repository,service,controller}` | Narrow published-Post resolution and tag-overlap reads. | Read-only; no cross-capability write transaction. | **Canonical implementation and read-only dependency contracts converged.** |
| **PostVersion** | Post version history/restore; `post_versions` | `internal/postversion/{domain,repository,service,controller}` plus `internal/postversion.RestoreCoordinator` | Approval uses narrow `postVersionReader`; restore writes Post state only through the Post-owned `RestoreSnapshotTx` port. `PostVersion.Status` references the Post-owned status contract under `internal/post/domain`. | `RestoreCoordinator` owns the atomic snapshot-read/Post-write transaction through `dbtx.Transactor`; repositories receive the caller transaction. | **Canonical implementation, restore ownership, and domain model ownership converged.** |
| **Provider** | AI Provider profiles and encrypted credential metadata; `ai_provider_profiles` | `internal/provider` plus `internal/provider/{domain,repository}` | Agent ManagementService consumes the canonical Provider repository through a narrow provider-store contract. | Provider repository for owned writes. | **Management consumer and domain model ownership converged.** The flat Provider facade is retired; credential validation/encryption/security behavior is unchanged. |
...
| **Access** | Blog principals, identity aliases, memberships/roles and access policy; includes `blog_principals`, `blog_principal_identities` and access-control tables | `internal/access` plus `internal/access/controller` | Middleware/controller consume access snapshots. Identity key is `(issuer, subject)`. | Access service owns security-sensitive SQL transaction boundaries. | **Canonical ownership.** Security behavior remains protected and unchanged. |
...
| **Shared DB infrastructure** | Transaction execution and SQL error classification; no business tables | `internal/dbtx`, `internal/dberror` | Capabilities, including Connector, depend directly on these infrastructure packages. | Application/service coordinators decide transaction scope; `dbtx` only executes it. | **Canonical.** No root repository compatibility alias remains. |
| **Connector (HOLD: behavior/features)** | Connector profiles/outbox/audits; `ai_connector_profiles`, `ai_connector_outbox`, `ai_connector_delivery_audits` | `internal/connector` plus `internal/connector/controller` | Uses shared `controllerutil` and direct `dbtx.Transactor`; remains isolated from core Blog business flows. | Connector service owns its existing transaction behavior through canonical `dbtx`. | **Structural ownership converged.** Product behavior/features remain under Connector Module Hold. |

## Composition-root convergence

`cmd/gouno/web.go` is the sole application composition root. Canonical Post, Page, Taxonomy, Site, Community, Media, Analytics, Recommendation, PostVersion, Access, Agent, Knowledge, Operations and Connector dependencies are constructed there. Shared transaction infrastructure is constructed as `dbtx.Transactor` and injected directly into every consumer that requires it, including Connector.
...
- **Connector:** consumes `dbtx.Transactor` directly. Transaction behavior is unchanged; the product-behavior Hold remains independent of structural ownership.
...
None. The root `internal/repository` Transactor alias, flat Connector controller shell, and flat response helper facade are retired. Shared primitives are consumed directly from `internal/dbtx` and `internal/controllerutil`.

## Flat-layer classification

There are no remaining root business-layer buckets. `internal/domain`, `internal/service`, `internal/repository`, and `internal/controller` are terminally retired. Capability-local packages own business models, persistence, services, and HTTP transport; only proven horizontal infrastructure remains outside capability ownership.

## Remaining protected behavior boundaries

There is no remaining Capability Module structural migration debt.

1. **Connector Module Hold:** only product/behavior changes remain protected. Structural ownership is canonical; future Connector feature work still requires an explicit instruction and must preserve OAuth/callback, credential, outbox/delivery, Sandbox and isolation contracts unless intentionally redesigned.
2. **Access/Auth BFF security behavior:** structural ownership is canonical. Authorization, MFA, owner-transfer, session/BFF and browser-security semantics remain protected and require explicit security-focused review for behavioral changes.
3. **Architecture guard maintenance:** terminal guards must prevent recreation of root `internal/domain`, `internal/service`, `internal/repository`, or `internal/controller`. New compatibility facades are not a normal extension mechanism.

Completed slices: **Agent Approval / Media Candidate / Workflow Event orchestration**, **Agent ManagementService dependency cutover**, **Starter Pack application coordination**, **Runner dependency/lifecycle convergence**, **Workflow Definition/Version persistence extraction**, **Workflow Run lifecycle coordination**, **Agent MediaCandidate / Workflow Run boundary convergence**, and **Workflow Run admission/retry/recovery convergence**, **Workflow dispatch/execution persistence convergence**, and **Workflow read-model classification**, and **Workflow HTTP ownership convergence**, and **Operations HTTP ownership convergence**, and **Agent HTTP ownership convergence**, and **Knowledge HTTP ownership convergence**, and **PostVersion restore ownership convergence**, and **Feed HTTP ownership convergence**, and **flat-controller boundary freeze**, and **Page domain alias retirement**, and **Operations domain model ownership convergence**, and **Taxonomy domain model ownership**, and **Analytics read-model ownership**, and **Media domain model ownership**, and **PostVersion domain model ownership**, and **Post domain model ownership**, and **Workflow domain model ownership**, and **Provider domain model ownership**, and **Knowledge EmbeddingProfile domain ownership**, and **ToolRisk domain ownership**, and **Agent domain ownership / root-domain retirement**, and **flat-controller compatibility surface shrink** now use canonical ownership boundaries and explicit composition-root wiring. The flat `repository.AgentRepository` aggregate and its Agent/Workflow/Notification delegates are retired; the root `internal/repository` and `internal/controller` buckets are now fully removed after Connector direct-`dbtx` cutover and Access/Connector transport relocation.


### Final flat-boundary retirement — 2026-09-14

- Access HTTP transport moved from the generic root controller bucket to `internal/access/controller` with unchanged routes, middleware, MFA/authorization/owner-transfer behavior and response semantics.
- Connector HTTP transport moved from the misleading flat `AgentController` shell to `internal/connector/controller`; router/composition naming now reflects Connector ownership directly.
- Connector service and integration tests consume `internal/dbtx.Transactor` directly; the root `internal/repository.Transactor` compatibility alias is deleted.
- The root response forwarding facade is deleted; Access and Connector controllers consume `internal/controllerutil` directly.
- Root `internal/controller` and `internal/repository` are physically absent. Terminal non-existence guards prevent either generic bucket from returning.
- Connector OAuth/callback, credential, outbox/delivery, Sandbox behavior and route contracts are intentionally unchanged; only structural ownership changed.


### Flat-controller compatibility surface shrink — 2026-09-14

- The root `internal/controller` pagination facade and duplicate facade regression test are retired after consumer proof; pagination implementation and tests remain canonical under `internal/controllerutil`.
- The root response facade now exposes only `ParamPositiveID` and `WriteDomainError`, the two helpers still consumed by the stable Access boundary and/or Connector-held transport. Unused `ParamInt64`, pagination, validation, formatting, and service-error forwards are retired.
- `internal/controllerutil/flat_controller_boundary_test.go` now allowlists only Access, the Connector transitional shell/transport, and the minimal held response facade.
- Access and Connector route bodies, middleware, OAuth/callback behavior, credentials, response contracts, and security semantics are unchanged.
...
### Workflow domain model ownership — 2026-09-14
...
### PostVersion domain model ownership — 2026-09-14
...
### Media domain model ownership — 2026-09-14
...
### Analytics read-model ownership — 2026-09-14
...
### Taxonomy domain model ownership — 2026-09-14
...
### Operations domain model ownership convergence — 2026-09-14
