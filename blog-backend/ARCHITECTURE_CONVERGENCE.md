...
- **Workflow:** `RunLifecycle` owns cancellation/deletion; `Service.RetryFailed` owns its raw transaction copying Workflow run/resources/steps. Service no longer needs a Transactor for cancellation/deletion.
...
1. **Workflow consolidation:** Definition/version persistence is now isolated. Cancellation/deletion lifecycle coordination is now explicit. Next design queue/retry/recovery and execution checkpoint persistence as coherent contracts, including resources and event scheduling; do not extract isolated SQL statements.
...
Completed slices: **Agent Approval / Media Candidate / Workflow Event orchestration**, **Agent ManagementService dependency cutover**, **Starter Pack application coordination**, **Runner dependency/lifecycle convergence**, and **Workflow Definition/Version persistence extraction** now use canonical ownership boundaries and explicit composition-root wiring. The flat `repository.AgentRepository` aggregate and its Agent/Workflow/Notification delegates are retired; `internal/repository` remains only for the Connector-held Transactor alias until that Hold is explicitly lifted.
