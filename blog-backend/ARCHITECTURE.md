...
Post Version persistence, business behavior, and active HTTP ownership are fully canonical under `internal/postversion/{repository,service,controller}`. The repository owns version listing and transactional restore persistence, with direct integration coverage under the capability. The service owns identifier validation and missing-version error semantics. The application composition root constructs the canonical Post Version service directly, `WebRouterOptions.PostVersionSvc` carries it explicitly, and both `/api/admin/posts/:id/versions` and `/api/admin/posts/:id/versions/:versionID/restore` bind to the canonical Post Version controller while preserving the existing `PostPolicy` authorization checks. Shared HTTP error mapping recognizes canonical Post Version sentinels. Agent Approval depends only on a narrow `postVersionReader` contract for the version lookup it actually needs instead of depending on an aggregate service.

Blog Tools no longer store or depend on any Growth service. `NewBlogRegistry` temporarily retains an ignored, untyped third argument only as call-shape compatibility for existing tests and internal callers; that placeholder carries no capability object, behavior, or dependency and may be removed in a later API-cleanup change without affecting capability ownership.
