...
Analytics persistence and business behavior are now canonical under `internal/analytics/{repository,service}`. The repository owns `analytics_events` writes and the cross-capability analytics read model spanning posts, comments, reports, notifications, and daily event counts. The service owns view-recording validation and event semantics. The flat `GrowthRepository` and `GrowthService` retain Analytics methods only as compatibility delegates while controller/runtime ownership migrates in a later stage; the flat service explicitly preserves the historical `ErrInvalidPostID` identity at that compatibility boundary. Post-version and recommendation behavior remain in the flat Growth bucket until the later Post capability migration.
