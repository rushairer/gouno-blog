...
The next legacy content bucket is being decomposed by real ownership rather than by filename. Category and Tag form the `taxonomy` capability, while site settings belong to the separate `site` capability. Canonical persistence lives under `internal/taxonomy/repository` and `internal/site/repository`; canonical business behavior now lives under `internal/taxonomy/service` and `internal/site/service`. The old `internal/repository/category_repository.go` and `internal/service/category_service.go` remain temporary composite facades only for the still-flat controller/router wiring. HTTP ownership migrates after these repository and service seams are proven by full CI.
