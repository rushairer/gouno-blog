...
Recommendation persistence and business behavior are canonical under `internal/recommendation/{repository,service}`. The repository owns the tag-overlap query and related-post row mapping used by `/api/posts/:slugOrID/related`, with direct integration coverage under the capability. The service owns source-post validation, the empty-tag fast path, and the bounded related-post query limit. The flat `GrowthRepository` and `GrowthService` retain `RelatedPosts` only as compatibility delegates while Recommendation controller/runtime ownership migrates; the flat service preserves the historical `service.ErrPostNotFound` identity at that compatibility boundary. Post-version persistence and behavior remain in the flat Growth bucket until their own capability seam is established.
