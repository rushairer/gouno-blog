...
Root content models such as `domain.Category`, `domain.TagSummary`, `domain.Post`, and `domain.PostVersion` are not moved merely for directory symmetry. Their model boundary must be decided from actual ownership and cross-capability usage before any later relocation.

The legacy flat `GrowthRepository` / `GrowthService` / `GrowthController` bucket is not promoted wholesale into a fake `growth` capability. It historically mixed recommendations, post-version restore, analytics, and media behavior, and decomposition follows real ownership. Media-asset persistence, business behavior, and HTTP ownership are fully canonical under `internal/media/{repository,service,controller}`. Active Media routes use the canonical controller/service, SVG upload security lives with that HTTP adapter, shared HTTP error mapping recognizes canonical Media sentinels, and Agent generation/approval depend on narrow Media create/list contracts. The former GrowthRepository/GrowthService Media compatibility delegates and legacy Media error shims have been removed after full repository compilation proved no remaining consumers.

Analytics is fully canonical under `internal/analytics/{repository,service,controller}`. The application composition root constructs the Analytics repository/service directly and reuses that canonical service for every active runtime consumer. `WebRouterOptions.AnalyticsSvc` carries the service explicitly to the canonical Analytics controller, while `BindAnalytics` connects the public `analytics.get_summary` Tool definition directly to the same service. The unbound Tool placeholder fails closed instead of falling back to Growth. `GrowthRepository` and `GrowthService` no longer own, implement, delegate, store, or expose any Analytics persistence or business behavior. The Analytics repository owns `analytics_events` writes and the cross-capability read model spanning posts, comments, reports, notifications, and daily event counts; the Analytics service owns view-recording validation and event semantics; the Analytics controller owns both `/api/posts/:slugOrID/view` and `/api/admin/analytics`.

Recommendation persistence, business behavior, and active HTTP ownership are canonical under `internal/recommendation/{repository,service,controller}`. The repository owns the tag-overlap query and related-post row mapping used by `/api/posts/:slugOrID/related`, with direct integration coverage under the capability. The service owns source-post validation, the empty-tag fast path, and the bounded related-post query limit. The application composition root constructs the canonical Recommendation service directly, `WebRouterOptions.RecommendationSvc` carries it explicitly, and the public related-post route binds to the canonical Recommendation controller. Shared HTTP error mapping recognizes the Recommendation not-found sentinel directly. The flat `GrowthController` no longer exposes RelatedPosts HTTP behavior. `GrowthRepository.RelatedPosts` and `GrowthService.RelatedPosts` remain only as short-lived compatibility delegates pending final removal of the legacy Growth bucket.

Post Version persistence, business behavior, and active HTTP ownership are canonical under `internal/postversion/{repository,service,controller}`. The repository owns version listing and transactional restore persistence, with direct integration coverage under the capability. The service owns identifier validation and missing-version error semantics. The application composition root constructs the canonical Post Version service directly, `WebRouterOptions.PostVersionSvc` carries it explicitly, and both `/api/admin/posts/:id/versions` and `/api/admin/posts/:id/versions/:versionID/restore` bind to the canonical Post Version controller while preserving the existing `PostPolicy` authorization checks. Shared HTTP error mapping recognizes canonical Post Version sentinels. Legacy Growth repository/service objects remain temporarily instantiated only for non-HTTP Agent/Tool compatibility; they no longer own any active HTTP route. The next cleanup slice replaces those remaining consumers with narrow capability contracts and removes the flat Growth repository/service/controller files entirely.
