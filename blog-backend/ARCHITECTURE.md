...
Analytics persistence, business behavior, and active HTTP ownership are now canonical under `internal/analytics/{repository,service,controller}`. The repository owns `analytics_events` writes and the cross-capability analytics read model spanning posts, comments, reports, notifications, and daily event counts. The service owns view-recording validation and event semantics. The canonical Analytics controller owns both `/api/posts/:slugOrID/view` and `/api/admin/analytics`; view tracking resolves the route's `slugOrID` through the published-post boundary before recording an event, removing the previous flat Growth controller's mismatched `id` parameter assumption. The flat `GrowthRepository` and `GrowthService` retain Analytics methods only as compatibility delegates for remaining non-HTTP consumers such as Blog Tools, and the flat service explicitly preserves the historical `ErrInvalidPostID` identity at that compatibility boundary. Post-version and recommendation behavior remain in the flat Growth bucket until their own seams are established.
