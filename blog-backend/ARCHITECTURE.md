...
Analytics persistence is now canonical under `internal/analytics/repository`. That repository owns `analytics_events` writes and the cross-capability analytics read model spanning posts, comments, reports, notifications, and daily event counts. The flat `GrowthRepository` retains `RecordEvent` and `AnalyticsSummary` only as compatibility delegates while Analytics service/controller/runtime ownership migrates in later stages. Post-version and recommendation persistence remain in the flat Growth bucket until their own seams are established.
