...
Recommendation persistence is canonical under `internal/recommendation/repository`. It owns the tag-overlap query and related-post row mapping used by `/api/posts/:slugOrID/related`, with direct integration coverage under the capability. The flat `GrowthRepository` retains `RelatedPosts` only as a compatibility delegate while Recommendation service/controller/runtime ownership migrates in later stages. Post-version persistence and behavior remain in the flat Growth bucket until their own capability seam is established.
