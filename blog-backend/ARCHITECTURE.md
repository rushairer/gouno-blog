...
The next legacy content bucket is being decomposed by real ownership rather than by filename. Category and Tag persistence form the `taxonomy` capability, while site settings persistence belongs to a separate `site` capability. Canonical repositories live under `internal/taxonomy/repository` and `internal/site/repository`; the old `internal/repository/category_repository.go` is a temporary composite facade for the still-flat service/controller layer. Service and HTTP ownership migrate only after this persistence seam is proven by full CI.
