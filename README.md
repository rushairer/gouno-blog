...
所有 Compose 镜像默认已固定到验证时的不可变摘要。更新自建服务时，应先验证新镜像，再以 `tag@sha256:<digest>` 覆盖对应变量；不要使用浮动分支标签：

```bash
export GOUNO_BLOG_BACKEND_IMAGE_TAG=sha-...@sha256:...
export GOUNO_BLOG_FRONTEND_IMAGE_TAG=sha-...@sha256:...
export GOUNO_BLOG_SEED_IMAGE_TAG=sha-...@sha256:...
export GOSSO_IMAGE_TAG=sha-...@sha256:...
export GOSSO_ADMIN_FRONTEND_IMAGE_TAG=sha-...@sha256:...
export GOSSO_ADMIN_SEED_IMAGE_TAG=sha-...@sha256:...
```
...
- 媒体候选：批准的图片 brief 会进入媒体库候选区，记录来源 Run、模型、token 用量与 alt 文本提案；管理员必须同时通过安全与版权检查，候选才可进入生成流程。将生成结果按普通媒体上传后，才可关联回已审核候选；系统不自动发布。
