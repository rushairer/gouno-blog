...
RSS 已通过白名单 HTTPS Tool 提供受限读取，站点 Sitemap 为现有只读公开源；Webhook 已提供 HMAC SHA-256 校验、1 MiB 限制和幂等键，生产环境必须配置 `GOUNO_AI_WEBHOOK_SECRET`。Search Console 已提供真实、只读的 Google OAuth 授权码流程、加密 refresh token 存储及聚合指标查询；Profile 需配置 HTTPS 回调地址、Client ID、站点 URL，并将 Client Secret 作为加密凭据保存。媒体存储可在本地文件系统与 S3 兼容对象存储之间切换，上传、AI 图片生成、删除和 URL 解析复用同一存储抽象。已实现本地 Sandbox 连接器底座及 AI 工作台管理界面：凭据经现有密钥环加密、模拟 OAuth 回调、审批后 Outbox、幂等键、每连接器分钟限流、带退避的失败重试、Mock 投递审计和撤销；界面明确显示无网络投递并支持 Profile、OAuth Mock、入队、审批、投递、重试和撤销。Mock Transport 明确不含网络请求。Newsletter 与社媒的真实适配仍需独立 OAuth/服务凭据和经确认的供应商配置；不提供任意 HTTP、任意代码或模型生成插件。
...
创建页只调用 `POST /api/admin/ai-workflows/draft`。规划器使用 `workflow-intent/v2` 将完整自然语言需求生成为类型化意图和 Workflow，并校验 Cron、用户输入、步骤数据流、Agent 能力、已注册 Tool 与人工审批边界；不再通过关键字匹配固定模板，也不再创建 Skill 或 Agent 草案。

- Workflow 草案默认停用，管理员审阅、保存并 Dry-run 后才能运行。
- 已保存的 starter Workflow 仍保留其最小运行前检查契约，以保证历史数据可安全执行；它们不是新的创建路径。
- Workflow `/preflight` 无副作用检查；Dry-run 和正式运行在入队前复用服务端校验输入 Schema、Agent 状态和只读 discovery 权限。
