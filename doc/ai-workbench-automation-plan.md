...
已实现可视化筛选器、保存预览、预计命中、上次实际命中、空结果策略、三类默认停用的规则化计划模板、逐项部分失败汇总、按资源重试（保留原输入与动态集合快照）、`for_each` 有界并发和资源类型配额；后续支持失败批量重试体验、事件触发和外部连接器。

### 第三阶段：事件触发

已实现 `post.published`、`post.updated`、`comment.created`、`comment.reported`、`media.uploaded` 和 `suggestion.created` 的幂等事件表、精确字段过滤、冷却检查、事件入队 API、数据库触发器以及带指数退避的失败重放；Scheduler 消费未处理事件。批处理窗口和 `link_check.failed` 仍待补齐。事件不能绕过 Skill、预算、Tool 白名单、范围或审批。
