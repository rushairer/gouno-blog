...
- [x] 先生成社媒/newsletter/FAQ 草稿和图片 brief；草稿只进入审批记录供人工审阅和复制，不连接外部服务；外部发布始终单独审批。
- [~] 图片 brief 已进入媒体库候选区，带来源 Run、模型、token 用量、alt 文本提案及默认“未检查”的版权/安全状态；管理员必须同时通过安全与版权检查，候选才能进入生成流程。生成结果可按现有媒体安全上传流程入库后关联回候选；自动图片生成与货币成本仍待独立 Provider 安全评审后实现。
