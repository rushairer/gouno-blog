...
### 阶段 8：站点知识与引用式创作（进行中，3–4 周）

目标：提供站点级上下文，而不是孤立的“写作按钮”。

- 为已授权文章建立异步、可重建的分段检索索引；索引失败不得阻塞页面访问或发布。
- [x] 新增 `content.find_internal_links` 基线 Tool：以共享标签、标题/关键词匹配为可解释依据，排除已有链接，只返回已发布文章。
- 新增语义 `content.find_related` Tool，返回文章 ID、标题、摘要、相关片段和来源位置。
- 将 Agent 输出中的站内依据结构化为 citations；界面可跳转原文，并明确标出无来源的建议。
- 增加索引任务队列、版本/删除同步、租户与权限隔离，以及检索质量离线评测集。

验收：编辑 Agent 能为长文给出可点击的内链和来源；无权限/已删除内容永不出现在结果中；P95 检索延迟和索引积压均有监控。

### 阶段 9：Skill 与确定性 Workflow（3–4 周）

目标：把反复成功的 Agent 操作沉淀为可治理、可复用的“无代码插件”。

- 增加 versioned Skill 定义：允许的 Tool、固定指令、输入 schema、触发器、预算和审批策略。
- 支持保存 Agent 运行配置为私有 Skill、从内置模板创建、导入/导出 JSON；首版不开放任意公网市场安装。
- 将发布前检查、周报、旧文刷新做成可重复执行的 Workflow；模型调用只位于需要判断的节点，数据搬运和通知使用确定性节点。
- 增加 Skill 版本锁定、dry-run、执行 diff、回滚/停用和按 Skill 的成本、失败率统计。

验收：一次 Workflow 运行可完整复现输入、版本、Tool 调用和审批结果；升级 Skill 不会改变历史运行语义。

### 阶段 10：运营闭环与主动建议（2–3 周）

目标：从“等待提问”升级到管理员可控制的运营提醒。

- 以聚合指标识别高曝光低互动、长期未更新、失效链接、孤岛文章和标签膨胀。
- 创建站内运营任务/通知提案，默认不发送外部消息；支持摘要周报和优先级排序。
- 对标题、摘要和封面文案支持受控 A/B 候选及人工选择；指标读取只使用脱敏聚合数据。
- 引入反馈标签（采纳/拒绝/无效），形成提示词、Skill 和规则的离线改进依据。
