...
状态：verified（Q01a/Q01b 已修复，本地完整门禁通过；尚未集成）。前置：阅读 [路线图](../roadmap.md) 与 [基线](../baseline.md)。
...
## Q01b 最终续验（2026-09-05）

本地阻断解除。完整数据库门禁在两个独立一次性容器均通过：31 个必跑（核心 30、Connector 原测试 1）全部通过，零失败/跳过/缺失；race 开启，三个负向探针均按预期失败，容器正常清理。原两项 Workflow 失败修复，新增串行/并发断点恢复与幂等验证。详见 [Q01b 证据与限制](Q01b-workflow-iterations.md)。

后端全仓 race/vet、前端 148/148 与构建、门禁脚本单测、actionlint、diff 检查通过。此前各次失败保留为历史证据。Q01 标记本地 verified；远端 CI、Linux 冷启动与制品上传仍待后续验证，不标 integrated。

Q02 前置状态：已满足本地验收；需包含 c1c6158 的前置提交与当前未提交 Q01b 修复/计划文档，不能仅凭 HEAD 开新工作区。此次无推送、合并、发布或部署。
