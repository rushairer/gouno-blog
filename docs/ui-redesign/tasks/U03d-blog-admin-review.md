...
- 当前 `main` 已推进到 `b5be525965eab771a1999b8e351bb02e1ef8cf6d`；本轮只把能够由 Showcase、源码与自动化明确证明的漂移归回产品实现，不把静态审查冒充浏览器验收。
...
- Dependency ownership：`blog-frontend/package.json` 的基础 UI 依赖边界已收敛到 `@gouno/ui@0.3.5` + Tailwind；不再直接声明 Radix/shadcn-era primitive 依赖，lockfile 中的 `@radix-ui/*` 仅为发布版 `@gouno/ui` 的传递依赖。
...
代码级 Showcase reconciliation 与本轮静态封板检查已收口到 `b5be525965eab771a1999b8e351bb02e1ef8cf6d`：基础依赖、全局 CSS ownership、历史兼容 hook、公开 Blog 的主要 Showcase 漂移均已有主线事实或回归保护。当前剩余工作不再是继续扩大 UI 重构，而是 U03d 定义要求的独立 rendered/browser QA。
