...
The single canonical owner is [`rushairer/gouno-ui`](https://github.com/rushairer/gouno-ui). `blog-frontend` consumes its verified vendored artifact as `@gouno/ui` and imports product-agnostic primitives from the governed `@gouno/ui/core`, `@gouno/ui/theme`, `@gouno/ui/patterns`, and `@gouno/ui/gouno` entrypoints.
