...
- `blog-backend/internal/service` has been retired. `internal/domain` remains a deliberate migration boundary only for models whose shared/cross-capability role is still proven; do not retain capability-owned compatibility aliases once consumers can import the canonical domain without cycles. `internal/repository` is retained only for the Connector-held transaction alias, while `internal/controller` is frozen to the stable Access security boundary, Connector-held transport/shell, and explicitly allowlisted compatibility helpers. `internal/controllerutil/flat_controller_boundary_test.go` rejects unclassified additions. Do not add new business ownership to either flat package.
