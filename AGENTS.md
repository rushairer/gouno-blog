...
- `blog-backend/internal/service` and the former root `internal/domain` shared-model bucket are retired. Agent-owned models live under `internal/agent/domain`, and `internal/agent/domain/ownership_test.go` prevents recreation or imports of the retired root domain path. `internal/repository` is retained only for the Connector-held transaction alias, while `internal/controller` is frozen to the stable Access security boundary, Connector-held transport/shell, and explicitly allowlisted compatibility helpers. `internal/dbtx/flat_repository_boundary_test.go` and `internal/controllerutil/flat_controller_boundary_test.go` reject unclassified additions. Do not add new business ownership to either flat package.
...
- The initial non-held flat-layer convergence is complete. Do not reintroduce temporary business facades into retired/shared flat buckets. Any compatibility surface that must remain for a documented constraint must be narrowly consumed, explicitly classified, allowlist-guarded, and carry a concrete removal condition in `blog-backend/ARCHITECTURE_CONVERGENCE.md`. The stable Access boundary and Connector Module Hold are the only currently retained flat exceptions.
...
- Shared HTTP/controller primitives belong in `blog-backend/internal/controllerutil`; capability controllers may depend on this package. The legacy `internal/controller` allowlist is a frozen compatibility/security boundary, not a destination for new business controllers or ownership tests.
