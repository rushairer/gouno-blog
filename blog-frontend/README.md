...
Public releases require an exact npm SemVer and registry integrity. The current
Git dependency is a temporary release blocker until `@gosso/client@0.4.0` is
available through the trusted publisher; it must not appear in a stable Blog
release. See [`../doc/auth-client-boundary.md`](../doc/auth-client-boundary.md).
