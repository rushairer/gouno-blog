...
interface Comment { id: number; post_id: number; author: string; content: string; status?: 'pending' | 'visible' | 'hidden'; is_visible: boolean; report_count?: number; created_at: string; }
...
export default function Admin() {
  ...
  const fetchModeration = useCallback(async () => {
    const query = new URLSearchParams({ status: moderationStatus, reported: String(reportedOnly) });
    const response = await apiFetch(`/api/admin/comments?${query}`);
    if (response.ok) setModerationComments((await response.json()).data?.list || []);
  ...
  const loadVersions = async (postID: number) => { const response = await apiFetch(`/api/admin/posts/${postID}/versions`); if (response.ok) setVersions((await response.json()).data || []); };
  const startCreate = () => { setEditingPost(null); setIsCreating(true); setVersions([]); setFormTitle(''); setFormSlug(''); setFormSummary(''); setFormContent(''); setFormTags(''); setFormStatus('draft'); setScheduledAt(''); dirtyRef.current = false; setModeratingPostId(null); };
  const startEdit = (post: Post) => { setEditingPost(post); setIsCreating(false); setFormTitle(post.title); setFormSlug(post.slug); setFormSummary(post.summary); setFormContent(post.content); setFormTags(post.tags.join(', ')); setFormStatus(post.status); setScheduledAt(toShanghaiInput(post.scheduled_at)); dirtyRef.current = false; setModeratingPostId(null); void loadVersions(post.id); };
  const cancel = () => { setEditingPost(null); setIsCreating(false); dirtyRef.current = false; };
  const restoreVersion = async (version: PostVersion) => { if (!editingPost || !confirm(t('restoreVersionConfirm'))) return; const response = await apiFetch(`/api/admin/posts/${editingPost.id}/versions/${version.id}/restore`, { method: 'POST' }); const body = await response.json(); if (!response.ok) { setError(body.message || t('requestFailed')); return; } const restored = body.data as Post; setEditingPost(restored); setPosts((items) => items.map((item) => item.id === restored.id ? restored : item)); setFormTitle(restored.title); setFormSlug(restored.slug); setFormSummary(restored.summary); setFormContent(restored.content); setFormTags(restored.tags.join(', ')); setFormStatus(restored.status); setScheduledAt(toShanghaiInput(restored.scheduled_at)); dirtyRef.current = false; void loadVersions(restored.id); };
  ...
  const moderateGlobal = async (comment: Comment, status: 'visible' | 'hidden') => { const response = await apiFetch(`/api/admin/comments/${comment.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status }) }); if (response.ok) setModerationComments((items) => items.filter((item) => item.id !== comment.id)); };
  const deleteGlobalComment = async (comment: Comment) => { if (!confirm(t('deleteCommentConfirm'))) return; const response = await apiFetch(`/api/comments/${comment.id}`, { method: 'DELETE' }); if (response.ok) setModerationComments((items) => items.filter((item) => item.id !== comment.id)); };
