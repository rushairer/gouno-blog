...
import { AdminPage, AdminPageHeader, ConfirmDialog, EmptyState, Feedback, LoadingState, Modal, Panel, useToast } from '../../components/ui';
...
export default function AdminTaxonomy({ type }: { type: 'categories' | 'tags' }) {
  ...
  return (
    <AdminPage>
      ...
      {type === 'categories' ? <Panel><form className="taxonomy-form" onSubmit={createCategory}><label>名称<input name="name" required /></label><label>Slug<input name="slug" className="mono" pattern="[a-z0-9]+(?:-[a-z0-9]+)*" required /></label><label>描述<input name="description" /></label><label>排序<input name="sort_order" type="number" defaultValue="0" /></label><button className="btn btn-primary"><Plus /> 新建分类</button></form></Panel> : null}
