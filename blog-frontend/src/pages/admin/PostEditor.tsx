...
import { AdminPageState, ConfirmDialog, Feedback, Select } from '../../components/ui';
  ...
  return <div className="editor-page">
    ...
    <div className="editor-workspace">
      ...
      <aside className="editor-inspector">
        <details open><summary>发布设置</summary><label>状态<Select value={publishIntent} onChange={(event) => { setPublishIntent(event.target.value as PostStatus); dirty.current = true; }}><option value="draft">草稿</option><option value="published">立即发布</option><option value="scheduled">定时发布</option></Select></label>{publishIntent === 'scheduled' ? <label>发布时间<input type="datetime-local" value={post.scheduled_at?.slice(0, 16) || ''} onChange={(event) => update('scheduled_at', event.target.value)} /></label> : null}</details>
