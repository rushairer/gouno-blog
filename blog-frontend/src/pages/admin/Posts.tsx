...
import { Copy, Edit2, Eye, Plus, Search, Trash2, X } from 'lucide-react';
...
export default function AdminPosts() {
  ...
  return (
    <AdminPage>
      ...
      <Panel className="admin-filter-bar">
        <div className="filter-search"><Search /><input aria-label="搜索文章" value={q} onChange={(event) => setFilter('q', event.target.value)} placeholder="搜索标题、摘要或正文" /></div>
        <select aria-label="文章状态" value={status} onChange={(event) => setFilter('status', event.target.value)}><option value="">全部状态</option><option value="published">已发布</option><option value="draft">草稿</option><option value="scheduled">定时发布</option></select>
        <select aria-label="文章分类" value={category} onChange={(event) => setFilter('category', event.target.value)}><option value="">全部分类</option>{categories.map((item) => <option key={item.id} value={item.slug}>{item.name}</option>)}</select>
        <select aria-label="文章标签" value={tag} onChange={(event) => setFilter('tag', event.target.value)}><option value="">全部标签</option>{tags.map((item) => <option key={item.name} value={item.name}>{item.name}</option>)}</select>
        <span>{total} 篇</span>
        {(q || status || category || tag) ? <button className="filter-clear" type="button" onClick={clearFilters}><X /> 清除</button> : null}
      </Panel>
      {selected.length ? <div className="bulk-action-bar"><strong>已选择 {selected.length} 篇</strong><button onClick={() => void batch('publish').catch((reason: Error) => setError(reason.message))}>发布</button><button onClick={() => void batch('draft').catch((reason: Error) => setError(reason.message))}>转为草稿</button><button className="danger-action" onClick={() => setDeleteTarget({ kind: 'batch' })}>删除</button><button onClick={() => setSelected([])}>取消</button></div> : null}
