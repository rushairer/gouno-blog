...
describe("PageEditor", () => {
  ...
  it("loads the page and uses DocumentEditorShell + MarkdownEditor without a post navigator", async () => {
    const getAdminPageSpy = vi.spyOn(pagesApi, "getAdminPage").mockResolvedValue(draftPage);
    ...
    expect(screen.getByLabelText("单页正文 Markdown")).toHaveValue(draftPage.content);
    ...
    expect(screen.queryByRole("button", { name: /Slug 候选/ })).not.toBeInTheDocument();
  });
  ...
  it("uses AISuggestionPicker for title and regenerates through the real AI endpoint", async () => {
    ...
    await user.type(await screen.findByLabelText("单页正文 Markdown"), "页面正文");
    await user.click(screen.getByRole("button", { name: "AI 生成标题候选" }));
    expect(await screen.findByRole("radio", { name: "关于 Gouno" })).toBeChecked();
    await user.click(screen.getByRole("button", { name: "重新生成 AI 建议" }));
    expect(await screen.findByRole("radio", { name: "重新生成的单页标题" })).toBeChecked();
  ...
  it("reviews slug and SEO together instead of exposing a standalone slug AI action", async () => {
    ...
    expect(screen.queryByRole("button", { name: /生成 Slug/ })).not.toBeInTheDocument();
    ...
    expect(screen.getByLabelText("SEO 描述")).toHaveValue("Gouno project introduction");
  });
  ...
  it("matches the post editor publish intent actions", async () => {
    ...
    await waitFor(() =>
      expect(updatePageSpy).toHaveBeenCalledWith(3, expect.objectContaining({ status: "published" })),
    );
  ...
  it("keeps unsaved page fields mounted across responsive reflow", async () => {
    ...
    const title = await screen.findByPlaceholderText("写一个清晰、具体的单页标题");
  ...
  it("saves before opening the frontsite preview", async () => {
    ...
    await user.type(await screen.findByPlaceholderText("写一个清晰、具体的单页标题"), "预览单页");
    ...
    await waitFor(() =>
      expect(createPageSpy).toHaveBeenCalledWith(
        expect.objectContaining({ title: "预览单页", slug: "preview-page", status: "draft" }),
      ),
    );
  ...
  it("shows backend save conflicts without masking them", async () => {
    const user = userEvent.setup();
    vi.spyOn(pagesApi, "getAdminPage").mockResolvedValue(draftPage);
    vi.spyOn(pagesApi, "updatePage").mockRejectedValue(new Error("单页已被其他编辑者更新（409 冲突）"));
    renderEditor("/admin/pages/3/edit");

    await screen.findByDisplayValue(draftPage.title);
    await user.click(screen.getByRole("button", { name: "保存草稿" }));
    expect(await screen.findAllByText("单页已被其他编辑者更新（409 冲突）")).not.toHaveLength(0);
  });
