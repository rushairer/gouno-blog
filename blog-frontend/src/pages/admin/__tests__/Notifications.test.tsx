...
describe("Admin Notifications Page", () => {
  ...
  it("marks a single notification as read", async () => {
    ...
    await waitFor(() => {
      expect(apiFetch).toHaveBeenCalledWith("/api/me/notifications/1/read", {
        method: "PUT",
      });
    });
  });
  ...
  it("deletes a single notification via confirmation modal", async () => {
    ...
    await waitFor(() => {
      expect(apiFetch).toHaveBeenCalledWith("/api/me/notifications/1", {
        method: "DELETE",
      });
    });
  });
  ...
  it("clears all read notifications via header action", async () => {
    ...
    const clearReadBtn = await screen.findByRole("button", {
      name: /清空已读/,
    });
    ...
    await waitFor(() => {
      expect(apiFetch).toHaveBeenCalledWith(
        "/api/me/notifications?only_read=true",
        { method: "DELETE" },
      );
    });
  });
