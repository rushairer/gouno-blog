...
vi.mock('../../../components/agent/WorkflowLauncher', () => ({
  WorkflowLauncher: ({ open, resourceKeys }: { open: boolean; resourceKeys: number[] }) => open ? <div>launcher:{resourceKeys.join(',')}</div> : null,
}));

describe('AdminComments structured AI input', () => {
  beforeEach(() => {
    vi.mocked(apiFetch).mockResolvedValue(Response.json({ data: { list: [{
      id: 17, post_id: 4, author: 'Reader', content: 'Could you explain this?', status: 'pending',
      is_visible: false, report_count: 0, created_at: '2026-08-02T00:00:00Z',
    }] } }));
  });

  it('passes selected comment IDs to the Workflow launcher', async () => {
    const user = userEvent.setup();
    render(<MemoryRouter><ToastProvider><AdminComments /></ToastProvider></MemoryRouter>);

    await user.click(await screen.findByRole('checkbox', { name: '选择评论 17' }));
