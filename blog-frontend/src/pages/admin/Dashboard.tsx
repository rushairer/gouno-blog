...
export default function Dashboard() {
  ...
  if (!allowed || (!summary && !error)) return <LoadingState label="正在进入内容工作台…" />;
  ...
  return <AdminPage>
    ...
    {summary ? <>
      <div className="admin-metrics">
        <Panel><FileText /><span>文章</span><strong>{summary.total_posts}</strong><small>已发布 {summary.published_posts}</small></Panel>
        <Panel><Eye /><span>总阅读</span><strong>{summary.total_views.toLocaleString()}</strong><small>所有已发布文章</small></Panel>
        <Panel><MessageSquare /><span>评论</span><strong>{summary.total_comments}</strong><small>待审核 {summary.pending_comments}</small></Panel>
        <Panel><Bookmark /><span>收藏</span><strong>{summary.total_bookmarks}</strong><small>累计收藏次数</small></Panel>
      </div>
