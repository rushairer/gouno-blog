...
import { AdminPage, AdminPageHeader, BulkActionBar, Button, Checkbox, ConfirmDialog, ContentStack, EmptyState, Feedback, LoadingState, Modal, Panel, useToast } from '../../components/ui';
import { WorkflowLauncher } from '../../components/agent/WorkflowLauncher';
import { useAdminGuard } from '../../hooks/useAdminGuard';

type TagEdit = { tag: TagSummary; mode: 'rename' | 'merge' } | null;
type DeleteTarget = { kind: 'tag'; item: TagSummary } | { kind: 'batch' } | null;
    ...
    try {
      if (deleteTarget.kind === 'batch') {
        ...
        if (failed.length) {
          ...
          setError(`已删除 ${removed.length} 个标签；${failed.length} 个未删除：${reason instanceof Error ? reason.message : '请稍后重试。'}`);
        } else {
  ...
  return (
    <AdminPage>
            ...
            {tags.map((tag) => (
              <Panel className="tag-admin-card" key={tag.name}>
                ...
                <div className="tag-admin-card__actions" aria-label={`标签 ${tag.name} 操作`}>
                  <Button variant="secondary" size="compact" type="button" onClick={() => setTagEdit({ tag, mode: 'rename' })}>
                    <Save />重命名
                  </Button>
                  <Button variant="secondary" size="compact" type="button" onClick={() => setTagEdit({ tag, mode: 'merge' })}>
                    <Merge />合并
                  </Button>
                  <Button variant="danger" size="compact" type="button" onClick={() => setDeleteTarget({ kind: 'tag', item: tag })}>
                    <Trash2 />删除
                  </Button>
                </div>
              </Panel>
      ...
      <Modal
        ...
        description={tagEdit?.mode === 'merge' ? `将“${tagEdit?.tag.name}”合并至目标标签。` : `为“${tagEdit?.tag.name}”输入新名称。`}
      ...
      >
        <form className="modal-form" onSubmit={saveTag}>
          ...
          <div className="modal-actions">
            <Button variant="secondary" type="button" onClick={() => setTagEdit(null)}>取消</Button>
      ...
      <ConfirmDialog
        ...
        description={
          deleteTarget?.kind === 'batch'
            ...
            : deleteTarget ? `从所有文章中移除标签“${deleteTarget.item.name}”？` : ''
        }
        confirmLabel="确认删除"
        danger
        ...
        onConfirm={remove}
      />
      <WorkflowLauncher open={aiOpen} resourceType="tag" resourceKeys={selected} onClose={() => setAIOpen(false)} title="将所选标签交给 AI" />
    </AdminPage>
  );
}
