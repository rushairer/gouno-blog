...
describe("NotFound Page", () => {
  it("renders 404 code, message, search bar, and navigation shortcuts", () => {
    ...
    expect(
      screen.getByPlaceholderText(/搜索文章或主题|Search articles or topics/),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /搜索|Search/ }),
    ).toBeInTheDocument();

    expect(screen.getByRole("link", { name: /回到首页|Home/ })).toHaveAttribute(
      "href",
      "/",
    );
    expect(
    ...
    expect(
      screen.getByRole("link", { name: /内容分类|Categories/ }),
    ).toHaveAttribute("href", "/categories");
    expect(
      screen.getByRole("link", { name: /全站归档|Archive/ }),
  ...
  it("allows typing a query and submitting search", async () => {
    ...
    const input = screen.getByPlaceholderText(
      /搜索文章或主题|Search articles or topics/,
    );
    ...
    expect(window.location.pathname + window.location.search).toContain(
      "/search?q=AI%20%E8%BF%90%E8%90%A5",
    );
  });
});
