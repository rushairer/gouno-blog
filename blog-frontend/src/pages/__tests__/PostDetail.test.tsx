...
describe("PostDetail", () => {
  ...
  it("loads by slug, renders markdown, and loads comments by resolved id", async () => {
          ...
          data: [
            {
              ...post,
              id: 8,
              title: "Related Go Post",
              slug: "related-go-post",
            },
          ],
        });
      }
    ...
    ).toHaveAttribute("src", "/media/diagram.jpg");
    expect(screen.getByRole("img", { name: "Cover image" })).toHaveAttribute(
      "src",
      "/media/cover.jpg",
    );
    ...
    ).toHaveAttribute("href", "/articles/related-go-post");
  ...
  it("posts a comment and shows the pending review notice", async () => {
    ...
    await user.type(
      screen.getByPlaceholderText(/type your comment/i),
      "Hello there",
    );
  ...
  it("renders a two-level thread and sends the selected parent id with a reply", async () => {
    ...
    await waitFor(() => {
      const request = fetchMock.mock.calls.find(
        ([input, init]) =>
          input.toString() === "/api/posts/7/comments" &&
          init?.method === "POST",
      );
  ...
  it("allows previewing a draft post by slug when preview=true", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "每日AI资讯：2026年8月21日" }),
    ).toBeInTheDocument();
