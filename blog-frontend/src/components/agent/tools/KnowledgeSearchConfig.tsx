  ...
  return (
    <div className="tool-config-card">
      <div className="tool-config-card__header">
        <div className="tool-config-card__icon"><DatabaseZap size={18} /></div>
        <div>
          <h4>{isZh ? '知识库检索设置 (content.search_knowledge)' : 'Knowledge Base Search (content.search_knowledge)'}</h4>
      ...
      <div className="tool-config-grid">
        <Field label={isZh ? '单次证据切片召回条数' : 'Evidence snippets limit'} hint={isZh ? '默认 8，范围 1~20 条' : 'Default 8, range 1-20'}>
          <input
            type="number"
            ...
            max={20}
            value={limit}
            onChange={(e) => onChange({ ...value, limit: Math.max(1, Math.min(20, Number(e.target.value) || 8)) })}
          />
        </Field>
      </div>
    </div>
