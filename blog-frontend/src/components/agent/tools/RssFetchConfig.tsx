...
export const POPULAR_RSS_PRESETS: Array<{ label: string; name: string; url: string }> = [
  { label: 'OpenAI News', name: 'OpenAI News', url: 'https://openai.com/news/rss.xml' },
  { label: 'Google Blog', name: 'Google Blog', url: 'https://blog.google/rss/' },
  { label: 'TechCrunch AI', name: 'TechCrunch AI', url: 'https://techcrunch.com/category/artificial-intelligence/feed/' },
  { label: 'Hacker News Top', name: 'Hacker News', url: 'https://hnrss.org/frontpage' },
  { label: 'Hugging Face Blog', name: 'Hugging Face', url: 'https://huggingface.co/blog/feed.xml' },
];
...
export function RssFetchConfig({
  ...
  const feeds = value?.feeds && value.feeds.length > 0 ? value.feeds : DEFAULT_RSS_FEEDS;
  ...
  const updateFeed = (index: number, field: 'name' | 'url', val: string) => {
    const nextFeeds = feeds.map((feed, i) => (i === index ? { ...feed, [field]: val } : feed));
    onChange({ ...value, feeds: nextFeeds, max_per_feed: maxPerFeed, max_items: maxItems });
  };
  ...
  const addFeed = (feed: RssFeedItem = { name: '', url: '' }) => {
    onChange({ ...value, feeds: [...feeds, feed], max_per_feed: maxPerFeed, max_items: maxItems });
  };

  const addPreset = (preset: { name: string; url: string }) => {
    if (feeds.some((f) => f.url.toLowerCase() === preset.url.toLowerCase())) return;
    onChange({ ...value, feeds: [...feeds, { name: preset.name, url: preset.url }], max_per_feed: maxPerFeed, max_items: maxItems });
  };
  ...
  return (
    <div className="tool-config-card">
          ...
          {POPULAR_RSS_PRESETS.map((preset) => {
            ...
            return (
              <button
                ...
                disabled={exists}
                title={exists ? (isZh ? '已添加' : 'Already added') : isZh ? '点击添加此源' : 'Add this feed'}
              >
                + {preset.label}
              </button>
            );
        ...
        {feeds.map((feed, idx) => (
          <div key={idx} className="rss-config-table__row">
            ...
            <Button
              ...
              variant="secondary"
              size="compact"
              aria-label={isZh ? `删除 ${feed.name || '此源'}` : `Delete feed ${feed.name || idx + 1}`}
              onClick={() => removeFeed(idx)}
              disabled={feeds.length <= 1}
              title={feeds.length <= 1 ? (isZh ? '至少保留一个源' : 'Keep at least one feed') : isZh ? '删除此源' : 'Remove feed'}
            >
              <Trash2 size={14} />
            </Button>
          </div>
      ...
      <div className="rss-config-card__footer">
        <Button type="button" variant="secondary" size="compact" onClick={() => addFeed()}>
          <Plus size={14} />
          {isZh ? '添加订阅源' : 'Add Feed'}
        </Button>
      </div>
      ...
      <div className="rss-config-limits">
        <Field label={isZh ? '单个源最大抓取条数' : 'Max entries per feed'} hint={isZh ? '默认 8，范围 1~20' : 'Default 8, range 1-20'}>
          <input
            type="number"
            className="input-field"
            min={1}
            max={20}
            value={maxPerFeed}
            onChange={(e) => onChange({ ...value, feeds, max_per_feed: Math.max(1, Math.min(20, Number(e.target.value) || 8)), max_items: maxItems })}
          />
        </Field>
        <Field label={isZh ? '全局总抓取条数上限' : 'Max total entries'} hint={isZh ? '默认 20，范围 1~50' : 'Default 20, range 1-50'}>
          <input
            type="number"
            className="input-field"
            min={1}
            max={50}
            value={maxItems}
            onChange={(e) => onChange({ ...value, feeds, max_per_feed: maxPerFeed, max_items: Math.max(1, Math.min(50, Number(e.target.value) || 20)) })}
          />
        </Field>
      </div>
    </div>
