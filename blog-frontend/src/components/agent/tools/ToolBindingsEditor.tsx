...
export function ToolBindingsEditor({
  ...
  const [jsonText, setJsonText] = useState(() => JSON.stringify(toolBindings || {}, null, 2));
  ...
  useEffect(() => {
    ...
    if (capabilities.includes('rss.fetch') && (!next['rss.fetch'] || !next['rss.fetch'].feeds)) {
      next['rss.fetch'] = { feeds: DEFAULT_RSS_FEEDS, max_per_feed: 8, max_items: 20 };
      updated = true;
    }
    if (capabilities.includes('content.list_stale_posts') && !next['content.list_stale_posts']) {
      next['content.list_stale_posts'] = { older_than_days: 180, limit: 20 };
      updated = true;
    }
    if (capabilities.includes('analytics.list_low_engagement_posts') && !next['analytics.list_low_engagement_posts']) {
      next['analytics.list_low_engagement_posts'] = { min_views: 100, max_engagement_rate: 0.02, limit: 20 };
      updated = true;
    }
    if (capabilities.includes('content.search_knowledge') && !next['content.search_knowledge']) {
      next['content.search_knowledge'] = { limit: 8 };
      updated = true;
    }
    if (capabilities.includes('content.propose_distribution_draft') && !next['content.propose_distribution_draft']) {
      next['content.propose_distribution_draft'] = { format: 'social', platform: 'Twitter' };
      updated = true;
    }
  ...
  const handleJsonChange = (text: string) => {
    ...
    try {
      ...
      if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
        setJsonError(isZh ? 'JSON 必须是对象格式（如 {"tool_name": { ... }}）' : 'JSON must be an object');
        return;
      }
      setJsonError(null);
      onChange(parsed as Record<string, Record<string, unknown>>);
    } catch (err: any) {
      setJsonError(err?.message || (isZh ? 'JSON 语法错误' : 'Invalid JSON syntax'));
    }
  };

  const hasRss = capabilities.includes('rss.fetch');
  const hasStale = capabilities.includes('content.list_stale_posts');
  const hasLowEngagement = capabilities.includes('analytics.list_low_engagement_posts');
  const hasKnowledge = capabilities.includes('content.search_knowledge');
  const hasDistribution = capabilities.includes('content.propose_distribution_draft');
  const hasAnySpecialized = hasRss || hasStale || hasLowEngagement || hasKnowledge || hasDistribution;

  if (configurableSelectedTools.length === 0 && Object.keys(toolBindings || {}).length === 0 && mode === 'visual') {
    return null;
  }
    ...
    <div className="tool-bindings-editor">
      <div className="tool-bindings-editor__toolbar">
        <div className="tool-bindings-editor__title">
          <Sliders size={16} />
          <strong>{isZh ? '受控 Tool 调用配置 (Tool Bindings)' : 'Controlled Tool Bindings'}</strong>
        </div>
        ...
        <div className="tool-bindings-editor__visual">
          {hasRss ? (
            <RssFetchConfig
              ...
              onChange={(nextRss) => onChange({ ...toolBindings, 'rss.fetch': nextRss as unknown as Record<string, unknown> })}
              locale={locale}
            />
          ) : null}
          ...
          {hasStale ? (
            <StalePostsConfig
              value={toolBindings['content.list_stale_posts'] as unknown as StalePostsBinding}
              onChange={(nextStale) => onChange({ ...toolBindings, 'content.list_stale_posts': nextStale as unknown as Record<string, unknown> })}
              locale={locale}
            />
          ) : null}
          ...
          {hasLowEngagement ? (
            <LowEngagementConfig
              value={toolBindings['analytics.list_low_engagement_posts'] as unknown as LowEngagementBinding}
              onChange={(nextLow) => onChange({ ...toolBindings, 'analytics.list_low_engagement_posts': nextLow as unknown as Record<string, unknown> })}
              locale={locale}
            />
          ) : null}
          ...
          {hasKnowledge ? (
            <KnowledgeSearchConfig
              value={toolBindings['content.search_knowledge'] as unknown as KnowledgeSearchBinding}
              onChange={(nextK) => onChange({ ...toolBindings, 'content.search_knowledge': nextK as unknown as Record<string, unknown> })}
              locale={locale}
            />
          ) : null}
          ...
          {hasDistribution ? (
            <DistributionDraftConfig
              value={toolBindings['content.propose_distribution_draft'] as unknown as DistributionDraftBinding}
              onChange={(nextDist) => onChange({ ...toolBindings, 'content.propose_distribution_draft': nextDist as unknown as Record<string, unknown> })}
              locale={locale}
            />
          ) : null}
