  ...
  return (
    <div className="tool-config-card">
      <div className="tool-config-card__header">
        <div className="tool-config-card__icon"><Calendar size={18} /></div>
        <div>
          <h4>{isZh ? '陈旧内容判定设置 (content.list_stale_posts)' : 'Stale Content Detection (content.list_stale_posts)'}</h4>
          ...
          {STALE_PRESETS.map((p) => {
            ...
            return (
              <button
                ...
                onClick={() => onChange({ ...value, older_than_days: p.days, limit })}
              >
                {p.label}
              </button>
            );
      ...
      <div className="tool-config-grid">
        <Field label={isZh ? '陈旧时间阈值 (天)' : 'Age threshold (days)'} hint={isZh ? '超过该天数未更新的文章将被判定为陈旧 (1~3650)' : 'Posts unedited for this many days (1-3650)'}>
          <input
            type="number"
            ...
            max={3650}
            value={olderThanDays}
            onChange={(e) => onChange({ ...value, older_than_days: Math.max(1, Math.min(3650, Number(e.target.value) || 180)), limit })}
          />
        </Field>
        <Field label={isZh ? '单次扫描最大篇数' : 'Max posts per scan'} hint={isZh ? '默认 20，范围 1~100' : 'Default 20, range 1-100'}>
          <input
            type="number"
            className="input-field"
            min={1}
            max={100}
            value={limit}
            onChange={(e) => onChange({ ...value, older_than_days: olderThanDays, limit: Math.max(1, Math.min(100, Number(e.target.value) || 20)) })}
          />
        </Field>
      </div>
    </div>
