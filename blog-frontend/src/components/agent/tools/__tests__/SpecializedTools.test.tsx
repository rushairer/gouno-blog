...
describe("Tool Config Components", () => {
  it("StalePostsConfig allows updating threshold days and using presets", async () => {
    ...
    render(
      <StalePostsConfig
        value={{ older_than_days: 180, limit: 20 }}
        onChange={onChange}
        locale="zh"
      />,
    );
    ...
    expect(onChange).toHaveBeenCalledWith(
      expect.objectContaining({ older_than_days: 90 }),
    );
  });
  ...
  it("LowEngagementConfig allows updating min_views and engagement rate", async () => {
    ...
    render(
      <LowEngagementConfig
        value={{ min_views: 100, max_engagement_rate: 0.02, limit: 20 }}
        onChange={onChange}
        locale="zh"
      />,
    );
    ...
    expect(onChange).toHaveBeenCalledWith(
      expect.objectContaining({ max_engagement_rate: 0.01 }),
    );
  });
  ...
  it("DistributionDraftConfig allows updating format and platform", async () => {
    ...
    render(
      <DistributionDraftConfig
        value={{ format: "social", platform: "Twitter" }}
        onChange={onChange}
        locale="zh"
      />,
    );
    ...
    expect(onChange).toHaveBeenCalledWith(
      expect.objectContaining({ format: "newsletter", platform: "Newsletter" }),
    );
  });
});
