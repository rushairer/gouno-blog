  ...
  return (
    <div className="tool-config-card">
      <div className="tool-config-card__header">
        <div className="tool-config-card__icon"><BarChart3 size={18} /></div>
        <div>
          <h4>{isZh ? '低互动分析阈值 (analytics.list_low_engagement_posts)' : 'Low Engagement Thresholds (analytics.list_low_engagement_posts)'}</h4>
          ...
          {ENGAGEMENT_PRESETS.map((p) => {
            ...
            return (
              <button
                ...
                onClick={() => onChange({ ...value, max_engagement_rate: p.rate, min_views: p.views, limit })}
              >
                {p.label}
              </button>
            );
      ...
      <div className="tool-config-grid">
        <Field label={isZh ? '最低阅读量门槛' : 'Minimum Views Cutoff'} hint={isZh ? '阅读量达到该数值的文章才纳入统计 (>= 1)' : 'Posts with at least this many views'}>
          <input
            type="number"
            className="input-field"
            min={1}
            value={minViews}
            onChange={(e) => onChange({ ...value, min_views: Math.max(1, Number(e.target.value) || 100), max_engagement_rate: maxRate, limit })}
          />
        </Field>
        <Field label={isZh ? '互动率预警红线 (%)' : 'Max Engagement Rate (%)'} hint={isZh ? '点赞+评论/阅读率低于该值被识别 (如 2 代表 2%)' : 'Identified when rate is below this %'}>
          <input
            type="number"
            step="0.1"
            min={0.1}
            ...
            onChange={(e) => onChange({ ...value, min_views: minViews, max_engagement_rate: Math.max(0.001, (Number(e.target.value) || 2) / 100), limit })}
          />
        </Field>
        <Field label={isZh ? '单次分析最大篇数' : 'Max posts per scan'} hint={isZh ? '默认 20，范围 1~100' : 'Default 20, range 1-100'}>
          <input
            type="number"
            className="input-field"
            min={1}
            max={100}
            value={limit}
            onChange={(e) => onChange({ ...value, min_views: minViews, max_engagement_rate: maxRate, limit: Math.max(1, Math.min(100, Number(e.target.value) || 20)) })}
          />
        </Field>
      </div>
    </div>
