...
export const PLATFORM_PRESETS = [
  ...
  { label: 'Email Newsletter', format: 'newsletter' as const, platform: 'Newsletter' },
  { label: '常见问题 (FAQ)', format: 'faq' as const, platform: 'Website FAQ' },
  { label: '封面配图 (Image Brief)', format: 'image_brief' as const, platform: 'Image Generator' },
];
  ...
  return (
    <div className="tool-config-card">
      <div className="tool-config-card__header">
        <div className="tool-config-card__icon"><Share2 size={18} /></div>
        <div>
          <h4>{isZh ? '多渠道内容分发设置 (content.propose_distribution_draft)' : 'Multi-Channel Distribution (content.propose_distribution_draft)'}</h4>
          ...
          {PLATFORM_PRESETS.map((p) => {
            ...
            return (
              <button
                ...
                onClick={() => onChange({ ...value, format: p.format, platform: p.platform })}
              >
                {p.label}
              </button>
            );
      ...
      <div className="tool-config-grid">
        <Field label={isZh ? '分发格式类型' : 'Distribution format'}>
          <Select
            value={format}
            onChange={(e) => onChange({ ...value, format: e.target.value as any, platform })}
          >
            <option value="social">{isZh ? '社交媒体推文 (Social)' : 'Social post'}</option>
            <option value="newsletter">{isZh ? '邮件周刊 (Newsletter)' : 'Email newsletter'}</option>
            <option value="faq">{isZh ? '文章 FAQ 问答 (FAQ)' : 'FAQ summary'}</option>
            <option value="image_brief">{isZh ? '配图设计方案 (Image Brief)' : 'Image brief'}</option>
          </Select>
        </Field>
        <Field label={isZh ? '目标平台标识' : 'Target platform name'} hint={isZh ? '如 Twitter, 小红书, 即刻, 邮件等' : 'e.g. Twitter, Substack, LinkedIn'}>
          <input
            type="text"
            className="input-field"
            value={platform}
            placeholder="Twitter"
            onChange={(e) => onChange({ ...value, format, platform: e.target.value })}
          />
        </Field>
      </div>
    </div>
