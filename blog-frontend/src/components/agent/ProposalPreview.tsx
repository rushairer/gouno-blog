...
export function ProposalPreview({
  ...
  const isDraft =
    actionType === "create_draft" || actionType === "create_page_draft";
  const isPage =
    actionType === "create_page_draft" || actionType === "update_page";

  return (
    <section
      className="proposal-preview"
      aria-label={zh ? "内容提案预览" : "Content proposal preview"}
    >
      ...
      {slug || tags.length > 0 ? (
        <dl className="proposal-preview__metadata">
          {slug ? (
            <div>
              <dt>Slug</dt>
          ...
          {tags.length > 0 ? (
            <div>
              <dt>{zh ? "标签" : "Tags"}</dt>
              <dd className="proposal-preview__tags">
                {tags.map((tag) => (
      ...
      {content ? (
        ...
        <p className="proposal-preview__unchanged">
          {zh
            ? "本次提案不修改正文。"
            : "This proposal does not change the body content."}
        </p>
      )}
    </section>
  );
