...
export function SkillForm({ initial, tools, locale, onSave, onCancel }: {
  ...
  return <EditorPanel title={labels.title} icon={<ListChecks />} closeLabel={labels.cancel} onClose={onCancel}><FormLayout onSubmit={submit}>
    ...
    <fieldset className="agent-limit-grid"><legend className="sr-only">{labels.limits}</legend><Field label="Max steps"><input className="input-field" type="number" min="1" max="20" value={value.max_steps} onChange={(event) => setValue((current) => ({ ...current, max_steps: Number(event.target.value) }))} /></Field><Field label={locale === 'zh' ? '默认日运行上限' : 'Default daily limit'}><input className="input-field" type="number" min="1" value={value.default_daily_run_limit} onChange={(event) => setValue((current) => ({ ...current, default_daily_run_limit: Number(event.target.value) }))} /></Field><Field label="Max input tokens"><input className="input-field" type="number" min="1" value={value.max_input_tokens} onChange={(event) => setValue((current) => ({ ...current, max_input_tokens: Number(event.target.value) }))} /></Field><Field label="Max output tokens"><input className="input-field" type="number" min="1" value={value.max_output_tokens} onChange={(event) => setValue((current) => ({ ...current, max_output_tokens: Number(event.target.value) }))} /></Field><Field label={locale === 'zh' ? '默认月 Token 预算' : 'Default monthly token budget'}><input className="input-field" type="number" min="1" value={value.default_monthly_token_budget} onChange={(event) => setValue((current) => ({ ...current, default_monthly_token_budget: Number(event.target.value) }))} /></Field></fieldset>
