...
export function SkillForm({ initial, prefill, tools, locale, onSave, onCancel }: {
  ...
  const labels = locale === 'zh' ? { title: initial ? '编辑 Skill' : '创建 Skill', name: '名称', description: '说明', prompt: '固定指令', mode: '执行模式', advisory: '仅分析建议', approval: '生成审批提案', triggers: '允许触发器', manual: '手动触发', cron: '定时触发', capabilities: '工具授权', limits: '默认治理限制', cancel: '取消', save: '保存 Skill', saving: '保存中…' } : { title: initial ? 'Edit Skill' : 'Create Skill', name: 'Name', description: 'Description', prompt: 'Fixed instructions', mode: 'Execution mode', advisory: 'Advisory only', approval: 'Create approval proposals', triggers: 'Allowed triggers', manual: 'Manual', cron: 'Scheduled', capabilities: 'Tool authorization', limits: 'Default governance limits', cancel: 'Cancel', save: 'Save Skill', saving: 'Saving…' };
  ...
  return <EditorPanel title={labels.title} icon={<ListChecks />} closeLabel={labels.cancel} onClose={onCancel}><FormLayout onSubmit={submit}>
    ...
    <ToolBindingsEditor capabilities={value.capabilities} tools={tools} toolBindings={value.tool_bindings} onChange={(bindings) => setValue((current) => ({ ...current, tool_bindings: bindings }))} locale={locale} />
