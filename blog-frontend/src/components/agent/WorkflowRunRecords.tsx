...
import type { Workflow, WorkflowRun, WorkflowStepRun } from '../../agent';
...
export function WorkflowRunRecords({ locale, workflows, runs, formatDateTime }: {
  locale: 'en' | 'zh';
  workflows: Workflow[];
  runs: WorkflowRun[];
  ...
  const inspect = async (run: WorkflowRun) => {
    ...
    setError('');
    try {
      const steps = await readData<WorkflowStepRun[]>(await apiFetch(`/api/admin/ai-workflow-runs/${run.id}/steps`));
  ...
  return <div className="workflow-records section-stack">
    ...
    <div className="agent-split-view">
      <Panel className="agent-master-panel agent-run-list">{filtered.length === 0 ? <EmptyState label={zh ? '还没有 Workflow 运行记录。' : 'No Workflow runs recorded yet.'} /> : filtered.map((run) => <button className={selected?.run.id === run.id ? 'active' : ''} key={run.id} type="button" onClick={() => void inspect(run)}><span className={`run-icon run-icon--${run.status}`}><GitBranch /></span><span><strong>{names.get(run.workflow_id) || `Workflow #${run.workflow_id}`}</strong><small>{formatDateTime(run.started_at || run.created_at)} · {run.dry_run ? 'Dry-run' : (run.schedule_key ? (zh ? `计划 ${run.schedule_key}` : `Scheduled ${run.schedule_key}`) : (zh ? '手动运行' : 'Manual'))}</small></span><span><b>{loadingID === run.id ? (zh ? '载入中' : 'Loading') : run.status.replace('_', ' ')}</b><ChevronRight /></span></button>)}</Panel>
      <Panel className="agent-detail-panel">{selected ? <div className="section-stack">
        <div className="panel-heading"><div><h2>{names.get(selected.run.workflow_id) || `Workflow #${selected.run.workflow_id}`}</h2><small>Run #{selected.run.id} · Workflow v{selected.run.workflow_version_id}</small></div><span className={`status-pill status-pill--${selected.run.status}`}>{selected.run.status.replace('_', ' ')}</span></div>
        ...
        <section className="workflow-step-log"><div className="panel-heading"><div><h3>{zh ? '步骤日志' : 'Step logs'}</h3><small>{zh ? '按实际执行顺序排列；展开查看输入、输出与错误。' : 'Ordered by execution time. Expand for input, output and errors.'}</small></div></div>{selected.steps.length === 0 ? <EmptyState label={zh ? '该运行没有步骤日志。' : 'This run has no step logs.'} /> : selected.steps.map((step, index) => <details key={step.id} open={step.status === 'failed'}><summary><span>{index + 1}</span><div><strong>{step.step_id}</strong><small>{step.step_type}{step.iteration !== undefined ? ` · #${step.iteration}` : ''}</small></div><span className={`status-pill status-pill--${step.status}`}>{step.status}</span><small>{duration(step.started_at, step.finished_at)}</small></summary><div className="workflow-step-log__body"><div><small>{zh ? '开始 / 结束' : 'Start / finish'}</small><p>{formatDateTime(step.started_at)} → {step.finished_at ? formatDateTime(step.finished_at) : '—'}</p></div>{step.error_message ? <div className="workflow-run-error"><small>{zh ? '错误' : 'Error'}</small><p>{step.error_message}</p></div> : null}<div><small>{zh ? '输入' : 'Input'}</small><JsonLog value={step.input} /></div><div><small>{zh ? '输出' : 'Output'}</small><JsonLog value={step.output} /></div></div></details>)}</section>
