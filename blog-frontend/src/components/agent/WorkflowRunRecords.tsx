  ...
  return <div className="workflow-records section-stack">
    ...
    <div className="agent-split-view">
      ...
      <Panel className="agent-detail-panel">{selected ? <div className="section-stack">
        ...
        {selected.candidates.length > 0 ? <section className="workflow-interactions"><div className="panel-heading"><div><h3>{zh ? '图片候选' : 'Image candidates'}</h3><small>{zh ? '候选属于本次运行；确认应用后会创建文章新版本。' : 'Candidates belong to this run; applying creates a new article version.'}</small></div></div>{selected.candidates.map((candidate) => <div className="workflow-interaction" key={candidate.id}>{candidate.media_asset_url ? <img className="workflow-candidate-preview" src={candidate.media_asset_url} alt={candidate.alt_text || candidate.headline} /> : null}<div><strong>{candidate.headline || `Candidate #${candidate.id}`}</strong><small>{candidate.generation_status} · {candidate.selected ? (zh ? '已选择' : 'selected') : (zh ? '未选择' : 'not selected')} · {candidate.placement || 'cover'}</small>{candidate.generation_status === 'generating' && candidate.generation_deadline_at ? <small>{zh ? `生成中，截止 ${formatDateTime(candidate.generation_deadline_at)}` : `Generating until ${formatDateTime(candidate.generation_deadline_at)}`}</small> : null}{candidate.error_message ? <small>{candidate.error_message}</small> : null}</div><div className="agent-row-actions">{candidate.generation_status === 'generated' && !candidate.selected ? <button className="btn btn-secondary" type="button" onClick={() => void candidateAction(candidate, 'select')}>{zh ? '选择' : 'Select'}</button> : null}{candidate.generation_status === 'generated' && candidate.selected && !candidate.applied_version_id ? <button className="btn btn-primary" type="button" onClick={() => void candidateAction(candidate, 'apply')}>{zh ? '确认应用' : 'Apply to article'}</button> : null}{candidate.generation_status === 'failed' || candidate.generation_status === 'ready_to_generate' || candidate.generation_status === 'cancelled' ? <button className="btn btn-secondary" type="button" onClick={() => void candidateAction(candidate, 'regenerate')}>{zh ? '重新生成' : 'Regenerate'}</button> : null}{candidate.generation_status === 'generating' ? <button className="btn btn-secondary" type="button" onClick={() => void cancelGeneration(candidate)}>{zh ? '取消生成' : 'Cancel generation'}</button> : null}</div></div>)}</section> : null}
