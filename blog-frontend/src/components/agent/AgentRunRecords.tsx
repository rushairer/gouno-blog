...
export function ContentAudit({ value, locale }: { value: unknown; locale: 'en' | 'zh' }) {
        ...
        {checks.length === 0 ? (
          ...
          <ul>
            {checks.map((check, index) => (
              <li key={`${check.code}-${index}`} className={`content-audit__check--${check.severity || 'info'}`}>
...
export function InternalLinkSuggestions({ value, locale }: { value: unknown; locale: 'en' | 'zh' }) {
        ...
        <ul>
          {suggestions.map((suggestion) => (
            <li key={suggestion.post_id || suggestion.slug}>
              <div>
                <a href={`/articles/${encodeURIComponent(suggestion.slug || '')}`} aria-label={`${labels.open}: ${suggestion.title || suggestion.slug}`}>
              ...
              <div className="internal-link-suggestions__meta">
                <span>{labels.score} {suggestion.score || 0}</span>
                {suggestion.match_hints?.length ? <small>{labels.evidence}: {suggestion.match_hints.join(' · ')}</small> : null}
              </div>
            </li>
          ))}
        </ul>
...
export function RelatedContentSuggestions({ value, locale }: { value: unknown; locale: 'en' | 'zh' }) {
          ...
          {suggestions.map((suggestion) => (
            <li key={suggestion.post_id || suggestion.slug}>
              <div>
                ...
                {suggestion.snippet ? <p>{suggestion.snippet}</p> : suggestion.summary ? <p>{suggestion.summary}</p> : null}
                {suggestion.tags?.length ? <small>{suggestion.tags.join(' · ')}</small> : null}
              </div>
...
export function StalePostSuggestions({
  ...
  return (
    <section className="stale-post-suggestions" aria-label={labels.title}>
      <div className="content-audit__heading">
        <h3>{labels.title}</h3>
        {result.olderThanDays ? <small>{result.olderThanDays} days</small> : null}
      </div>
        ...
        <ul>
          {result.posts.map((post) => (
            <li key={post.id || post.slug}>
              <div>
                <a href={`/articles/${encodeURIComponent(post.slug || '')}`} aria-label={`${labels.open}: ${post.title || post.slug}`}>
                  {post.title || post.slug}
                </a>
                {post.summary ? <p>{post.summary}</p> : null}
              </div>
              <small>
                {labels.updated}: {post.updated_at ? formatDateTime(post.updated_at) : '—'} · {post.views_count || 0} {labels.views} · {post.likes_count || 0} {labels.likes}
              </small>
            </li>
          ))}
        </ul>
...
export function OrphanPostSuggestions({ value, locale }: { value: unknown; locale: 'en' | 'zh' }) {
  const result = stalePosts(value);
  if (!result) return null;
        ...
        <ul>
          {result.posts.map((post) => (
            <li key={post.id || post.slug}>
              ...
              <small>
                {post.views_count || 0} {locale === 'zh' ? '浏览' : 'views'} · {post.likes_count || 0} {locale === 'zh' ? '点赞' : 'likes'}
              </small>
            </li>
          ))}
        </ul>
...
export function RunCitations({ run, locale }: { run: AgentRun; locale: 'en' | 'zh' }) {
  if (!run.citations?.length) return null;
  return (
    <Panel>
      <section className="related-content-suggestions" aria-label={locale === 'zh' ? '引用依据' : 'Citations'}>
            ...
            <li key={citation.citation_id}>
              <div>
                {citation.status === 'validated' && citation.slug ? (
                  <a href={`/articles/${encodeURIComponent(citation.slug)}`}>{citation.title || citation.slug}</a>
                ) : (
...
export function RecordEvidence({
  ...
  const zh = locale === 'zh';
  return (
    <section className="record-evidence" aria-label={zh ? '本次运行的执行日志' : 'Execution log for this run'}>
      <div className="record-evidence__heading">
        <div>
          <h3>{zh ? '本次运行的执行日志' : 'Execution log for this run'}</h3>
          <small>{zh ? '每一步都属于上方当前选中的运行；展开可查看输入、结果与错误信息。' : 'Every step belongs to the selected run above. Expand a step to inspect its input, result, and errors.'}</small>
        </div>
        <strong>{zh ? `${run.tool_calls.length} 步` : `${run.tool_calls.length} steps`}</strong>
      </div>
            ...
            <details className="tool-call-detail" key={call.id}>
              <summary>
                ...
                <div className="tool-call-meta">
                  {call.created_at ? <small className="tool-call-time">{formatDateTime(call.created_at)}</small> : null}
                  <RiskPill risk={call.risk_level} locale={locale} />
                </div>
              </summary>
              {call.tool_name === 'content.audit_post' ? <ContentAudit value={call.result} locale={locale} /> : null}
              {call.tool_name === 'content.find_internal_links' ? <InternalLinkSuggestions value={call.result} locale={locale} /> : null}
              {call.tool_name === 'content.find_related' ? <RelatedContentSuggestions value={call.result} locale={locale} /> : null}
              {call.tool_name === 'content.list_stale_posts' ? <StalePostSuggestions value={call.result} locale={locale} formatDateTime={formatDateTime} /> : null}
              {call.tool_name === 'content.list_orphan_posts' ? <OrphanPostSuggestions value={call.result} locale={locale} /> : null}
              ...
              <details className="tool-call-technical">
                <summary>{zh ? '查看技术详情' : 'View technical details'}</summary>
                <JsonPreview value={{ arguments: call.arguments, result: call.result, error: call.error_message }} />
              </details>
            </details>
          );
        })}
          ...
          <div className="section-stack">
            ...
            <div className="agent-run-metrics">
              <span>
                ...
                <strong>{selectedRun.run.input_tokens + selectedRun.run.output_tokens} tokens</strong>
              </span>
              <span>
