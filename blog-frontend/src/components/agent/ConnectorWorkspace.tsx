  ...
  return (
    <div className="connector-workspace section-stack">
      <Panel>
        ...
        {profiles.length === 0 ? (
          ...
          <div className="table-scroll">
            <table className="content-table agent-table">
              <thead>
                <tr>
                  <th>{zh ? "Profile" : "Profile"}</th>
                  <th>{zh ? "凭据" : "Credential"}</th>
                  <th>OAuth</th>
                </tr>
              </thead>
              <tbody>
                {profiles.map((profile) => (
                  <tr key={profile.id}>
                    <td>
                      ...
                      <small>
                        {profile.kind} ·{" "}
                        {profile.sandbox ? "sandbox" : "read-only Google"}
                      </small>
                    </td>
                    <td>
                      {profile.has_credential
                        ? `•••• ${profile.credential_last4 || ""}`
                        : zh
                          ? "未连接"
                          : "Not connected"}
                    </td>
        ...
        {state ? (
          <div className="connector-oauth-callback">
            <label>
              State
      ...
      <Panel>
        <PanelHeader
          ...
          description={
            zh
              ? "先审批，再进行不可外发的 Mock 投递；幂等键防止重复入队。"
              : "Approve before a non-network Mock delivery; idempotency keys prevent duplicate queue entries."
          }
        />
        <div className="connector-form-grid">
          <label>
            ...
            <table className="content-table agent-table">
              ...
              <tbody>
                {outbox.map((item) => (
                  <tr key={item.id}>
                    <td>#{item.id}</td>
                    <td className="mono">{item.idempotency_key}</td>
                    <td>
                      <span
