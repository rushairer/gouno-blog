...
export function EmbeddingForm({ initial, locale, onSave, onCancel }: {
  ...
  const labels = locale === 'zh' ? {
    ...
    title: initial ? 'Edit embedding profile' : 'Add embedding profile', name: 'Profile name',
    base: 'OpenAI-compatible Base URL', model: 'Embedding model', dimensions: 'Vector dimensions',
    key: 'API Key', keep: 'leave blank to keep the existing key', timeout: 'Request timeout (seconds)',
    enabled: 'Enable this index profile', cancel: 'Cancel', save: 'Save', saving: 'Saving…',
  };
  const [value, setValue] = useState<EmbeddingFormValue>(() => initial ? {
    id: initial.id, name: initial.name, base_url: initial.base_url, model: initial.model,
    dimensions: initial.dimensions, api_key: '', enabled: initial.enabled,
    request_timeout_seconds: initial.request_timeout_seconds,
  } : {
    name: '', base_url: 'https://api.openai.com/v1', model: 'text-embedding-3-small',
    dimensions: 1536, api_key: '', enabled: true, request_timeout_seconds: 60,
  });
