...
describe("WorkflowInputForm", () => {
          ...
          list: [
            {
              type: "post",
              key: "42",
              label: "Structured Workflow inputs",
              status: "published",
              version_token: "2026-08-02T00:00:00Z",
              metadata: {},
            },
          ],
          total: 1,
  ...
  it("renders schema controls and writes selected integer resource IDs", async () => {
    ...
    const { rerender } = render(
      <WorkflowInputForm
        schema={schema}
        value={{ post_ids: [], format: "review", include_links: false }}
        onChange={onChange}
      />,
    );
    ...
    await user.click(
      await screen.findByRole("button", { name: /Structured Workflow inputs/ }),
    );
    expect(onChange).toHaveBeenCalledWith(
      expect.objectContaining({ post_ids: [42] }),
    );

    rerender(
      <WorkflowInputForm
        schema={schema}
        value={{ post_ids: [42], format: "faq", include_links: false }}
        onChange={onChange}
      />,
    );
    expect(
      screen.getAllByText("Structured Workflow inputs").length,
    ).toBeGreaterThan(0);
  });
  ...
  it("marks saved resource references that are no longer available", async () => {
    ...
    expect(
      screen.getByText("有 1 项资源已删除或不可用，请移除后再运行。"),
    ).toBeInTheDocument();
  });
  ...
  it("supplies and renders an explicit schema default as a fixed template parameter", async () => {
    ...
    await waitFor(() =>
      expect(onChange).toHaveBeenCalledWith({ format: "image_brief" }),
    );
