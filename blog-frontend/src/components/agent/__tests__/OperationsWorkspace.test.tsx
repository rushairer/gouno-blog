...
describe("OperationsWorkspace", () => {
  ...
  it("converts evidence-backed suggestions and selects candidates through governed APIs", async () => {
    ...
    expect(apiFetch).toHaveBeenCalledWith(
      "/api/admin/ai-suggestions/7/convert",
      { method: "POST" },
    );

    await user.click(
      screen.getByRole("button", { name: "Choose and create approval" }),
    );
    expect(apiFetch).toHaveBeenCalledWith(
      "/api/admin/ai-candidates/8/select",
  ...
  it("keeps automatically resolved suggestions out of the decision queue", () => {
    ...
    expect(
      screen.getByText("目前没有需要你决定的运营建议。"),
    ).toBeInTheDocument();
    expect(
  ...
  it("reviews an approved image brief before exposing image generation", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: "审核通过，进入生成" }),
    );
    expect(apiFetch).toHaveBeenCalledWith(
      "/api/admin/ai-media-candidates/31/review",
      expect.objectContaining({
        method: "POST",
