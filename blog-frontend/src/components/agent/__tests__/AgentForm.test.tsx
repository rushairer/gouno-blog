...
describe('AgentForm', () => {
  ...
  it('retains historical skill version in dropdown and displays upgrade banner when newer version exists', () => {
    ...
    render(<AgentForm
      ...
      providers={[{ id: 1, name: 'DeepSeek', model: 'deepseek-v4-flash', enabled: true } as never]}
    ...
    expect(screen.getByRole('option', { name: /AI 每日资讯 · v2 \(当前绑定 · 旧版本\)/ })).toBeInTheDocument();
    expect(screen.getByRole('option', { name: /AI 每日资讯 · v4/ })).toBeInTheDocument();
    ...
    expect(screen.queryByText(/检测到该 Skill 已更新至 v4/)).not.toBeInTheDocument();
