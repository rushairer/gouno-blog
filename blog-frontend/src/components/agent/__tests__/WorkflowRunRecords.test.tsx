...
describe("WorkflowRunRecords", () => {
  ...
  it("loads and displays step-level input, output, timing, and failure logs", async () => {
    ...
    await waitFor(() =>
      expect(apiFetch).toHaveBeenCalledWith(
        "/api/admin/ai-workflow-runs/6/steps",
      ),
    );
    expect(apiFetch).toHaveBeenCalledWith(
      "/api/admin/ai-workflow-runs/6/resources",
    );
    expect(
      screen.getByText(
        "Provider output did not match the required Markdown format.",
      ),
    ).toBeInTheDocument();
        ...
        data: String(url).endsWith("/events")
          ? [
              ...
              },
              {
                ...
                payload: {},
                created_at: "2026-08-01T01:00:02Z",
              },
            ]
          : [],
      }),
  ...
  it("renders an AI output summary as readable Markdown and keeps raw data collapsible", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "运行结论" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "分类与标签建议" }),
    ).toBeInTheDocument();
    const summary = document.querySelector(".workflow-run-output__summary");
    expect(summary).toBeInTheDocument();
    expect(
      within(summary as HTMLElement).getByText(/将.*拆分为更具体/),
    ).toBeInTheDocument();
    expect(screen.getByText("查看原始运行数据")).toBeInTheDocument();
    expect(
      document.querySelector(".workflow-run-output__raw"),
  ...
  it("retries a failed resource iteration and refreshes records", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByRole("button", { name: "重试此资源" }),
      ).toBeInTheDocument(),
    );
  ...
  it("supports selecting and applying multiple image candidates in one run", async () => {
    const user = userEvent.setup();
    const candidate = (id: number, placement: string) => ({
      id,
      post_id: 42,
      generation_status: "generated",
      selected: false,
      placement,
      anchor: placement === "inline" ? "## Details" : "",
      media_asset_url: `/media/${id}.png`,
      headline: `Candidate ${id}`,
      alt_text: `Alt ${id}`,
    });
    vi.mocked(apiFetch).mockImplementation(async (url) => {
      if (String(url).endsWith("/steps")) return Response.json({ data: [] });
      if (String(url).endsWith("/resources"))
        return Response.json({ data: [] });
      if (String(url).endsWith("/interactions"))
        return Response.json({ data: [] });
      if (String(url).endsWith("/media-candidates"))
        return Response.json({
          data: [candidate(1, "cover"), candidate(2, "inline")],
        });
      ...
      if (String(url).includes("/preview"))
        return Response.json({
          data: {
            placement: "cover",
            image_url: "/media/1.png",
            version_matches: true,
            anchor_matches: true,
            cover_url: "/media/1.png",
    ...
    await user.click(screen.getByRole("button", { name: /AI 每日资讯/ }));
    await waitFor(() =>
      expect(screen.getAllByText("Candidate 1")[0]).toBeInTheDocument(),
    );
    expect(
      document.querySelector(".workflow-candidate-card"),
    ).toBeInTheDocument();
    ...
    await waitFor(() =>
      expect(apiFetch).toHaveBeenCalledWith(
        "/api/admin/ai-workflow-runs/6/media-candidates/select",
    ...
    await waitFor(() =>
      expect(apiFetch).toHaveBeenCalledWith(
        "/api/admin/ai-image-tasks/1/preview",
      ),
    );
    await user.click(screen.getAllByRole("button", { name: "预览文章" })[0]);
    expect(
      await screen.findByRole("dialog", { name: "文章预览" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "Preview heading" }),
    ).toBeInTheDocument();
  ...
  it("keeps an ungenerated image brief actionable inside its source run", async () => {
    ...
    vi.mocked(apiFetch).mockImplementation(async (url) => {
      if (
        String(url).endsWith("/steps") ||
        String(url).endsWith("/resources") ||
        String(url).endsWith("/interactions") ||
        String(url).endsWith("/events")
      )
        return Response.json({ data: [] });
      if (String(url).endsWith("/media-candidates"))
    ...
    await user.click(screen.getByRole("button", { name: /AI 每日资讯/ }));
    await user.type(
      await screen.findByRole("textbox", { name: "图片要求 9" }),
      "横版，不要人物",
    );
    ...
    await waitFor(() =>
      expect(apiFetch).toHaveBeenCalledWith(
        "/api/admin/ai-image-tasks/9/regenerate",
        expect.objectContaining({
          method: "POST",
          body: JSON.stringify({ instruction: "横版，不要人物" }),
        }),
      ),
    );
  });
  ...
  it("explains that an outdated image candidate cannot be applied", async () => {
    ...
    vi.mocked(apiFetch).mockImplementation(async (url) => {
      ...
      if (String(url).includes("/preview"))
        return Response.json({
          data: {
            placement: "cover",
            image_url: "/media/14.png",
            version_matches: false,
            anchor_matches: false,
            cover_url: "/media/14.png",
            content: "## Preview heading",
          },
        });
      return Response.json({ data: {} });
    ...
    expect(
      await screen.findByText("候选图与当前文章不一致"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("文章已在生成候选图后更新，不能直接应用旧候选图。"),
    ).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "确认应用" })).toBeDisabled();
    expect(
      screen.queryByRole("button", { name: "刷新预览" }),
  ...
  it("saves an inline placement and anchor before allowing a preview", async () => {
          ...
          data: [
            {
              ...
              generation_status: "generated",
              selected: true,
              placement: placementSaved ? "inline" : "cover",
              anchor: placementSaved ? "Google 动态" : "",
              headline: "Cover brief",
              alt_text: "Cover image",
            },
          ],
        });
    ...
    await user.click(screen.getByRole("button", { name: /AI 每日资讯/ }));
    await user.click(
      screen.getByRole("combobox", { name: "图片位置 15" }),
    );
    ...
    expect(
      screen.getByText("位置设置尚未保存；保存后才能预览或应用。"),
    ).toBeInTheDocument();
    ...
    await waitFor(() =>
      expect(apiFetch).toHaveBeenCalledWith(
        "/api/admin/ai-image-tasks/15/select",
        expect.objectContaining({
          method: "POST",
          body: JSON.stringify({ placement: "inline", anchor: "Google 动态" }),
        }),
      ),
    );
    await waitFor(() =>
      expect(screen.getByRole("button", { name: "预览文章" })).toBeEnabled(),
    );
    await user.click(screen.getByRole("button", { name: "预览文章" }));
    await waitFor(() =>
      expect(apiFetch).toHaveBeenCalledWith(
        "/api/admin/ai-image-tasks/15/preview",
      ),
    );
  });
          ...
          data: [
            {
              ...
              generation_deadline_at: new Date(
                Date.now() + 15 * 60_000,
              ).toISOString(),
            },
          ],
  ...
  it("supports rejecting an individual image candidate and batch rejecting candidates", async () => {
    ...
    vi.mocked(apiFetch).mockImplementation(async (url) => {
      if (
        ...
        return Response.json({ data: [] });
      if (String(url).endsWith("/media-candidates"))
        return Response.json({ data: [candidate(1), candidate(2)] });
      return Response.json({ data: {} });
    });
    ...
    await waitFor(() =>
      expect(apiFetch).toHaveBeenCalledWith(
        "/api/admin/ai-image-tasks/1/reject",
        expect.objectContaining({ method: "POST" }),
      ),
    );
    ...
    await waitFor(() =>
      expect(apiFetch).toHaveBeenCalledWith(
        "/api/admin/ai-workflow-runs/100/media-candidates/reject",
        expect.objectContaining({
          method: "POST",
