...
describe('SkillForm', () => {
  it('renders visual RSS feed configuration when rss.fetch is checked', async () => {
    ...
    expect(screen.queryByText('RSS 订阅源设置 (rss.fetch)')).not.toBeInTheDocument();
    ...
    expect(await screen.findByText('RSS 订阅源设置 (rss.fetch)')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /可视化配置/ })).toBeInTheDocument();
  ...
  it('allows switching between visual form and raw JSON mode', async () => {
    ...
    render(
      <SkillForm
        initial={{
          ...
          tool_bindings: {
            'rss.fetch': {
              feeds: [{ name: 'Tech Blog', url: 'https://tech.example.com/feed' }],
            },
          },
    ...
    expect(screen.getByDisplayValue('https://tech.example.com/feed')).toBeInTheDocument();
