...
export function WorkflowWorkspace({ workflows, runs, metrics, agents, tools = [], locale, onRun, onPreflight, onRefresh, onSave, onConfigureSkill, onConfigureAgent }: {
  ...
  onConfigureAgent?: (draft?: { name?: string; description?: string; provider_profile_id?: number; skill_version_id?: number }) => void;
}) {
      ...
      <div className="workflow-detail-view section-stack">
        ...
        {(() => {
          ...
          return <div className="section-stack">
            <PanelHeader
              ...
              actions={
                <div className="row-actions">
                  ...
                  <a className="btn btn-secondary" href={`/admin/ai-ops?tab=records&record=workflow&workflow=${workflow.id}`}>{locale === 'zh' ? '运行记录' : 'Run records'}</a>
            ...
            {feedback ? <Feedback type={feedback.type}><div className="workflow-run-feedback"><span>{feedback.message}</span>{feedback.runID ? <a className="btn btn-secondary" href={`/admin/ai-ops?tab=records&record=workflow&workflow=${workflow.id}&run=${feedback.runID}`}>{runFeedbackActionLabel(feedback.action || 'viewRun', locale)}</a> : null}</div></Feedback> : null}
            ...
            <div className="workflow-step-summary" aria-label={locale === 'zh' ? '步骤流程' : 'Workflow steps'}>
              {workflow.steps.map((step, index) => (
                <div className="workflow-step-summary__item" key={step.id}>
                  <span className="workflow-step-summary__badge">{index + 1}</span>
                  <strong className="workflow-step-summary__name">{step.name || step.id}</strong>
                  <span className="workflow-step-summary__type">{step.type}</span>
                  {index < workflow.steps.length - 1 ? (
                    <span className="workflow-step-summary__arrow" aria-hidden="true">→</span>
                  ) : null}
                </div>
              ))}
            </div>
...
const resourceQueryTypes = [
  ['post', '文章'], ['comment', '评论'], ['media_asset', '媒体'], ['operational_suggestion', '运营建议'], ['category', '分类'], ['tag', '标签'], ['page', '单页'],
] as const;
...
const resourceQueryFilters: Record<string, ResourceQueryFilter[]> = {
  ...
  page: [
    ...
    { key: 'show_in_nav', label: '导航显示', type: 'select', options: ['true', 'false'] },
    { key: 'updated_before_days', label: '距今未更新天数', type: 'number' },
    { key: 'updated_after', label: '更新于此后（UTC）', type: 'datetime-local' },
    { key: 'updated_before', label: '更新于此前（UTC）', type: 'datetime-local' },
    { key: 'created_after', label: '创建于此后（UTC）', type: 'datetime-local' },
    { key: 'created_before', label: '创建于此前（UTC）', type: 'datetime-local' },
  ],
};
...
function SchemaFieldBuilder({ schemaJSON, onChange }: { schemaJSON: string; onChange: (value: string) => void }) {
  ...
  return <section className="workflow-schema-builder"><div className="workflow-resource-query-heading"><div><strong>输入字段</strong><p>资源字段会自动显示为文章、评论或媒体等选择器。</p></div><Button variant="secondary" size="compact" type="button" onClick={add}><Plus />添加字段</Button></div>{Object.entries(properties).map(([key, property]) => {
    const resource = String(property['x-gouno-resource'] || ''); const isArray = property.type === 'array'; const type = String(property.type || 'string');
    const set = (next: Record<string, unknown>) => save({ ...schema, properties: { ...properties, [key]: next } });
    const enumValues = Array.isArray(property.enum) ? property.enum : [];
    const scalar = !resource && !isArray;
    return <article className="workflow-schema-field" key={key}>
      <div className="form-grid"><Field label="字段名"><input value={key} onChange={(event) => { const name = event.target.value.trim(); if (!name || name === key || properties[name]) return; const next = { ...properties, [name]: property }; delete next[key]; save({ ...schema, properties: next, required: (schema.required || []).map((item) => item === key ? name : item) }); }} /></Field><Field label="标题"><input value={String(property.title || '')} onChange={(event) => set({ ...property, title: event.target.value })} /></Field><Field label="类型"><Select value={isArray ? 'array' : type} disabled={Boolean(resource)} onChange={(event) => { const next: Record<string, unknown> = { ...property, type: event.target.value, ...(event.target.value === 'array' ? { items: { type: 'string' } } : {}) }; delete next.enum; delete next.default; set(next); }}><option value="string">字符串</option><option value="integer">整数</option><option value="number">数字</option><option value="boolean">布尔</option><option value="array">数组</option></Select></Field><Field label="资源类型"><Select value={resource} onChange={(event) => { const resourceType = event.target.value; if (!resourceType) { const next = { ...property }; delete next['x-gouno-resource']; delete next['x-gouno-widget']; set(next); return; } const keyType = resourceType === 'tag' ? 'string' : 'integer'; const next: Record<string, unknown> = { ...property, type: isArray ? 'array' : keyType, ...(isArray ? { items: { type: keyType } } : {}), 'x-gouno-resource': resourceType, 'x-gouno-widget': isArray ? 'entity-multi-select' : 'entity-select' }; delete next.enum; delete next.default; set(next); }}><option value="">普通字段</option><option value="post">文章</option><option value="comment">评论</option><option value="media_asset">媒体</option><option value="operational_suggestion">运营建议</option><option value="category">分类</option><option value="tag">标签</option><option value="page">单页</option></Select></Field></div>
      <div className="form-grid"><label className="checkbox-field"><input type="checkbox" checked={(schema.required || []).includes(key)} onChange={(event) => save({ ...schema, required: event.target.checked ? [...new Set([...(schema.required || []), key])] : (schema.required || []).filter((item) => item !== key) })} />必填</label>{(resource || isArray) ? <><Field label="最少数量"><input type="number" min="0" value={Number(property.minItems || 0)} onChange={(event) => set({ ...property, minItems: Number(event.target.value) || 0 })} /></Field><Field label="最多数量"><input type="number" min="1" value={Number(property.maxItems || 20)} onChange={(event) => set({ ...property, maxItems: Number(event.target.value) || 1 })} /></Field></> : null}<Button variant="ghost" size="compact" type="button" onClick={() => { const next = { ...properties }; delete next[key]; save({ ...schema, properties: next, required: (schema.required || []).filter((item) => item !== key) }); }}><Trash2 />删除</Button></div>
      {scalar ? <div className="form-grid"><Field label="枚举值" hint="用逗号分隔；留空表示不限制。"><input value={enumValues.join(', ')} onChange={(event) => { const values = event.target.value.split(',').map((item) => item.trim()).filter(Boolean).map((item) => schemaScalar(item, type)); const next = { ...property }; if (values.length) next.enum = values; else delete next.enum; if (next.default !== undefined && !values.some((item) => Object.is(item, next.default))) delete next.default; set(next); }} /></Field><Field label="默认值" hint="运行表单仅在未填写时使用此值。"><input value={property.default === undefined ? '' : String(property.default)} onChange={(event) => { const next = { ...property }; const raw = event.target.value.trim(); if (!raw) delete next.default; else next.default = schemaScalar(raw, type); set(next); }} /></Field></div> : null}
      <Field label="说明"><input value={String(property.description || '')} onChange={(event) => set({ ...property, description: event.target.value })} /></Field>
    </article>;
  })}</section>;
}
