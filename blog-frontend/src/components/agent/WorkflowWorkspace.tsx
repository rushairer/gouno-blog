...
export function WorkflowWorkspace({ workflows, runs, metrics, locale, onMutate, onSave }: {
  ...
  return <div className="section-stack">
    <div className="row-actions"><button className="btn btn-primary" type="button" onClick={() => setEditing('new')}><Plus />{labels.add}</button></div>
    {workflows.length === 0 ? <Panel><EmptyState label={labels.empty} /></Panel> : workflows.map((workflow) => {
      ...
      return <Panel key={workflow.id}><div className="panel-heading"><div><h2>{workflow.name}</h2><small>{workflow.description} · v{workflow.current_version}</small></div><div className="row-actions"><button className="btn btn-secondary" type="button" onClick={() => setEditing(workflow)}><GitCompareArrows />Edit</button><button className="btn btn-secondary" type="button" onClick={() => void loadVersions(workflow)}><History />{labels.versions}</button><button className="btn btn-secondary" type="button" onClick={() => void onMutate(`/api/admin/ai-workflows/${workflow.id}/${workflow.enabled ? 'disable' : 'enable'}`)}>{workflow.enabled ? <CirclePause /> : <Play />}{workflow.enabled ? labels.disable : labels.enable}</button></div></div>
        ...
        <div className="row-actions"><button className="btn btn-secondary" type="button" onClick={() => void onMutate(`/api/admin/ai-workflows/${workflow.id}/dry-run`, 'POST', { input: JSON.parse(inputText) })}><TestTube2 />{labels.dry}</button><button className="btn btn-primary" disabled={!workflow.enabled} type="button" onClick={() => void onMutate(`/api/admin/ai-workflows/${workflow.id}/run`, 'POST', { input: JSON.parse(inputText) })}><Play />{labels.run}</button></div>
