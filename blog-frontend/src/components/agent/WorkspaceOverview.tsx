  ...
  return (
    <div className="workspace-overview section-stack">
      <Panel className="workspace-overview__hero">
        <div>
          <h2>{zh ? '从一件想改善的事开始' : 'Start with what you want to improve'}</h2>
          <p>{zh ? 'AI 会找出机会、准备建议；发布、修改和生成始终由你决定。' : 'AI finds opportunities and prepares proposals. You decide every publish, edit, and generation.'}</p>
        </div>
        <Button variant="primary" type="button" onClick={() => onNavigate('automation')}>
          <GitBranch />{zh ? '查看自动化' : 'Explore automation'}
        </Button>
      </Panel>
      <section className="workspace-overview__summary" aria-label={zh ? '当前待办' : 'Current work'}>
        ...
        <button type="button" onClick={() => onNavigate('inbox')}>
          ...
          <span>{zh ? '条内容建议待处理' : 'content suggestions to review'}</span>
        </button>
        <button type="button" onClick={() => onNavigate('inbox')}>
          <Sparkles />
        ...
        <Panel>
          <div className="panel-heading">
            <div>
              ...
              <small>{zh ? '按影响与人工决策优先级排序。' : 'Sorted by impact and the decisions only you can make.'}</small>
            </div>
          </div>
          <div className="workspace-overview__next">
            <strong>{reviewCount ? (zh ? `有 ${reviewCount} 项工作等你决定` : `${reviewCount} items need your decision`) : (zh ? '当前没有需要你处理的事项' : 'Nothing needs your decision right now')}</strong>
            <p>{zh ? '先审阅 AI 准备好的建议；它不会自行修改博客内容。' : 'Review AI-prepared proposals first; it never changes blog content on its own.'}</p>
            <Button variant="secondary" type="button" onClick={() => onNavigate('inbox')}>
              <ShieldCheck />{zh ? '进入待我处理' : 'Open review queue'}
            </Button>
          </div>
        </Panel>
        <Panel>
          <div className="panel-heading">
            <div>
              ...
              <small>{zh ? `已启用 ${enabledWorkflows} 个自动化流程。` : `${enabledWorkflows} automations are enabled.`}</small>
            </div>
          </div>
          <ul className="workspace-overview__goals">
            <li><b>{zh ? '发布前检查' : 'Pre-publish checks'}</b><span>{zh ? '发现 SEO、链接和内容问题。' : 'Catch SEO, links, and content issues.'}</span></li>
            <li><b>{zh ? '旧文更新' : 'Refresh older posts'}</b><span>{zh ? '发现需要维护的文章。' : 'Find posts that need maintenance.'}</span></li>
            <li><b>{zh ? '运营周报' : 'Operations reporting'}</b><span>{zh ? '汇总值得关注的变化。' : 'Summarize changes worth attention.'}</span></li>
          </ul>
          <Button variant="secondary" type="button" onClick={() => onNavigate('automation')}>
            <Play />{zh ? '配置自动化' : 'Configure automation'}
          </Button>
        </Panel>
      </div>
    </div>
