...
describe("AppFeedbackProvider", () => {
  it(
    "preserves status/alert semantics and supports manual dismissal",
    async () => {
      ...
      await user.click(
        screen.getAllByRole("button", { name: "关闭提示" })[0],
      );
  ...
  it(
    "keeps duration-zero feedback visible while default feedback expires",
    () => {
      vi.useFakeTimers();
      render(
        <AppFeedbackProvider>
