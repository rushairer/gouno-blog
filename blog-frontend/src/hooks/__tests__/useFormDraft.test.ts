...
import { useFormDraft, getFormDraft, setFormDraft, clearFormDraft } from "../useFormDraft";
...
describe("useFormDraft", () => {
  ...
  it("persists draft on save and restores it correctly", () => {
    ...
    expect(getFormDraft("test-post")).toEqual({ title: "Draft Post", content: "Hello world" });
  ...
  it("automatically snapshots current form state when STEP_UP_MFA_REQUIRED_EVENT fires", () => {
    ...
    renderHook(() => useFormDraft("auto-save-key", state, undefined, { autoRestore: false }));
  ...
  it("auto-restores draft upon hook mount when a draft exists in sessionStorage", () => {
    setFormDraft("mount-key", { name: "Restored AI Provider", model: "gpt-4o" });
    ...
    renderHook(() => useFormDraft("mount-key", state, setState, { autoRestore: true }));
