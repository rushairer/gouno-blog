...
const routeCases = [
  ...
  {
    path: "/admin/posts/101/edit",
    identity: "Browser Acceptance Post",
    identityKind: "title",
    structure: "editor",
  },
  {
  ...
  {
    path: "/admin/pages/201/edit",
    identity: "Browser Acceptance Page",
    identityKind: "title",
    structure: "editor",
  },
];
    ...
    for (const routeCase of routeCases) {
      test(`${routeCase.path} ${viewport.name}px ${theme}`, async ({ page }, testInfo) => {
        ...
        if (routeCase.structure === "editor") {
          const editor = page.locator('[data-slot="document-editor-shell"]');
          await expect(editor).toBeVisible();
          await expect(
            editor.locator(':scope > [data-slot="card"]'),
          ).toHaveCount(1);
          await expect(
            editor.locator('[data-slot="document-editor-command-bar"]'),
          ).toBeVisible();
          await expect(
            editor.locator('[data-slot="document-editor-workspace"]'),
          ).toBeVisible();
          await expect(
            editor.locator('[data-slot="document-editor-canvas"]'),
          ).toBeVisible();
          await expect(
            editor.locator('[data-slot="document-editor-inspector"]'),
          ).toBeVisible();

          if (routeCase.identityKind === "title") {
            await expect(
              editor.getByRole("textbox", { name: "标题", exact: true }),
            ).toHaveValue(routeCase.identity);
          } else {
            await expect(
              editor.getByRole("button", {
                name: routeCase.identity,
                exact: true,
              }),
            ).toBeVisible();
          }
